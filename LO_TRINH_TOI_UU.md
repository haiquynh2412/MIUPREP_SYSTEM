...
- [~] **3.3.2. i18n framework — nền tảng dùng chung ĐÃ XONG, rollout từng app tiếp theo** *(12/06/2026 — Tạo `@miuprep/i18n` (engine thuần testable: createTranslator + interpolation `{param}` + persistence + fallback chain + `findMissingTranslationKeys` guard; React: LanguageProvider/useTranslation/LanguageToggle). miuphysics migrate sang engine chung (giữ dict riêng), parity vi/en 100%. Wire vào build order + CI + hook. README hướng dẫn rollout. Test: engine test PASS, full build 0 lỗi. **Portal đã bắt đầu externalize (12/06):** `apps/miuprep-portal/src/i18n.ts` (28 keys vi/en full parity, toggle VI/EN); 2 surface xong — màn hình auth (login+register, 22 chuỗi) + header/top-bar (role badge, logout, tagline). Test: tsc 0 lỗi, build PASS, QA smoke PASS, CI i18n package xanh trên Linux. **Còn lại (incremental, an toàn vì chuỗi chưa dịch vẫn render raw):** các surface portal còn lại (notifications, dashboards, admin panels) + 2 app desktop — làm dần từng màn hình theo pattern đã lập)*
