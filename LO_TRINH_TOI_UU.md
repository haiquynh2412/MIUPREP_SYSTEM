...
- [~] **2.2.3. Portal: typecheck + tách App.tsx thành routes/modules**
  ...
  - [~] Giai đoạn 2b: tách state/handlers của App.tsx thành modules theo role *(13/06/2026 — bước 1: trích 3 mảng seed data tĩnh (INITIAL_MATH_LESSONS, INITIAL_CASIO_TIPS, INITIAL_IMPORTED_EXAMS) ra `lib/portalSeedData.ts`; App.tsx 2674→2617 dòng, zero đổi hành vi. Bước 2 (13/06): trích `safeFilePart`→`lib/fileUtils.ts` + 7 hàm thuần derive SAT learning-event→`lib/satPracticeEvents.ts`; App.tsx 2617→2489 dòng. Mỗi bước: tsc/lint 0 lỗi, build PASS, QA 2/2 consoleErrors=[]. **Lưu ý hạ tầng: build `npm run build:app` (MIUPREP_SKIP_PUBLIC_COPY=1) = ~12-23s vs build full ~10 phút (copy 900MB ảnh) — dùng cho vòng dev.** Còn lại: trích các nhóm handler (admin/parent/content-editor) thành custom hooks)*
