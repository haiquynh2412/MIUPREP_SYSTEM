...
- [x] 2026-06-04: Them scored template practice cho Math/English Core trong portal: Open practice tao session tu guided/independent/mixed/quick-check cua lesson template, submit dap an ghi `practice_attempt` co `correct`, score, itemId, concept/skill/template metadata; QA script `npm run qa:portal-template-practice` pass tren UI that.
