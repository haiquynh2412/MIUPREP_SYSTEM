...
export interface MasteryShadowAudit {
  ...
  evidenceSource: BetaEvidenceSource;
  learnerReports: Pick<MasteryV2ShadowReport, "generatedAt" | "summary" | "studentFacingEnabled" | "recommendationPolicy">[];
  detail: string;
}
...
export interface EmpiricalDifficultyAudit {
  ...
  evidenceSource: BetaEvidenceSource;
  learnerReports: Pick<EmpiricalDifficultyShadowReport, "generatedAt" | "summary" | "calibrationPolicy" | "highStakesPlacementEnabled">[];
  detail: string;
}
...
export function buildBetaReadinessReport(input: BetaReadinessInput): BetaReadinessReport {
  ...
  const scopes = BETA_SCOPES.map((scope) => buildScopeReport(input.graph, scope, input.contentUnits, learners, telemetryEvents));
  ...
  const checklist = buildChecklist(scopes, learningEvents, diagnosticAudit, recommendationAudit, graphAdjustmentCandidates);
...
export function buildInternalBetaRunPlan(
  ...
  const cohorts = report.scopes.map((scope) => {
    ...
    return {
      ...
      title: scope.title,
      status: scope.status === "blocked" ? "blocked" : scope.learners >= targetLearners && scope.diagnosticStatus !== "blocked" ? "pass" : "watch",
      targetLearners,
      ...
      focus: scope.nextActions.length || scope.blockers.length ? [...scope.blockers, ...scope.nextActions] : ["Run diagnostic, focused practice, review and tutor feedback loops."],
...
export function buildChangeImpactReport(input: {
  ...
  const items = [
    ...
    buildRecommendationQualityChangeImpactItem(previous?.recommendationQualityAudit, current.recommendationQualityAudit),
    ...
    .sort((a, b) => statusRank(b.severity) - statusRank(a.severity) || changeTypeRank(b.changeType) - changeTypeRank(a.changeType) || a.id.localeCompare(b.id))
    .slice(0, 30);
...
export function buildWeeklyBetaCohortReview(input: {
  ...
  const graphBacklog = input.report.graphAdjustmentCandidates.map((candidate) => ({
    id: candidate.id,
    kind: candidate.kind,
  ...
  const repairReroutes = input.report.repairRerouteCandidates.map((candidate) => ({
  ...
  const nextActions = uniqueStrings([
    ...
    ...graphBacklog.filter((item) => item.decision !== "defer").map((item) => `Review graph backlog ${item.id}: ${item.reason}`),
    ...repairReroutes.filter((item) => item.decision === "review").map((item) => `Review stuck repair path for ${item.learnerId} on ${item.targetId}: ${item.reason}`),
  ]).slice(0, 12);
  const statuses = [
  ...
  return {
    ...
    repairReroutes,
    decisions,
    nextActions: nextActions.length ? nextActions : ["Continue the current beta cohorts and collect next-week learning evidence."],
  };
}
...
export function auditDiagnosticValidity(
  ...
  const targetCoverage = average(
    BETA_SCOPES.map((scope) => calculateScopeTargetCoverage(graph, scopedUnits.filter((unit) => matchesScopeContent(unit, scope)), scope)),
  );
  const invalidTargetIds = uniqueStrings(scopedUnits.flatMap((unit) => [...(unit.conceptIds || []), ...(unit.skillIds || [])]).filter((id) => !hasGraphNode(graph, id)));
  ...
  learners.forEach((learner) => {
    ...
    if (missingTargets.length || outsideProgram.length) {
      issues.push({
        ...
        reason: missingTargets.length ? "recommendation_target_missing_graph_node" : "recommendation_target_outside_program_map",
        recommendationKind: recommendation.kind,
        targetIds,
      });
...
export function buildRealLearningKpiReport(
  ...
  const masteryLift = average(liveLearners.map((learner) => learnerMasteryLift(learner)).filter((value) => Number.isFinite(value)));
  ...
  const status: BetaCheckStatus = attempts.length === 0
    ? "blocked"
...
export function buildMasteryShadowAudit(learners: BetaLearner[] = [], generatedAt: string = new Date().toISOString()): MasteryShadowAudit {
  const learnersWithState = learners.filter((learner) => learner.state);
  const reports = learnersWithState.map((learner) => computeMasteryV2ShadowReport(learner.state!, { generatedAt, now: generatedAt }));
  ...
  const protectedFeedbackOnlyEvents = reports.reduce((sum, report) => sum + report.summary.protectedFeedbackOnlyEvents, 0);
  const evidenceSource = mergeEvidenceSources(learnersWithState.map(learnerEvidenceSource));
  const status: BetaCheckStatus = learnersWithState.length === 0 || rows.length === 0
    ? "watch"
...
export function buildEmpiricalDifficultyAudit(learners: BetaLearner[] = [], generatedAt: string = new Date().toISOString()): EmpiricalDifficultyAudit {
  const learnersWithState = learners.filter((learner) => learner.state);
  const reports = learnersWithState.map((learner) => buildEmpiricalDifficultyShadowReport(learner.state!, { generatedAt, minAttemptsPerItem: 5 }));
  const evidenceSource = mergeEvidenceSources(learnersWithState.map(learnerEvidenceSource));
  ...
  const status: BetaCheckStatus = learnersWithState.length === 0 || rows === 0
    ? "watch"
...
export function auditRecommendationQuality(graph: KnowledgeGraph, learners: BetaLearner[] = []): RecommendationQualityAudit {
  ...
  const status: BetaCheckStatus = checkedLearners === 0 ? "watch" : blockedIssues.length ? "blocked" : watchIssues.length ? "watch" : "pass";
  return {
    status,
    checkedLearners,
...
function calculateRetentionAfterSevenDays(learners: BetaLearner[]): number {
  ...
  learners.forEach((learner) => {
    ...
    attempts.forEach((attempt, index) => {
      const previousWrong = attempts.slice(0, index).reverse().find((previous) => {
...
function calculateLessonCompletionQuality(attempts: AttemptRecord[]): number {
  const learningAttempts = attempts.filter((attempt) => ["lesson", "practice", "review", "mock_test"].includes(normalizeText(attempt.mode)));
...
function auditRecommendationGraphScope(
  ...
  if (missingTargets.length) {
    issues.push(recommendationIssue(learner, recommendation, missingTargets, "missing_graph_target", "blocked", "Recommendation points to graph nodes that do not exist."));
  }
  if (outsideProgram.length) {
    issues.push(recommendationIssue(learner, recommendation, outsideProgram, "out_of_program_target", "blocked", "Recommendation target is outside the learner's assigned program map."));
  }
  return issues;
}
...
function auditRecommendationPrerequisites(
  ...
  return uniqueStrings(targetIds.flatMap((targetId) => graph.edges.filter((edge) => edge.to === targetId && edge.type === "prerequisite").map((edge) => edge.from)))
    ...
    .slice(0, 3)
    .map((prerequisiteId) => recommendationIssue(
      learner,
      recommendation,
      [prerequisiteId],
      "prerequisite_not_stable",
...
function auditRecommendationDifficulty(
  ...
  const averageTargetScore = targetRows.length ? average(targetRows.map((row) => row.score)) : average(mastery.map((row) => row.score));
  ...
  if (recommendation.kind === "diagnostic" && attempts >= 12) {
    return [recommendationIssue(learner, recommendation, targetIds, "stale_diagnostic", "watch", "Learner already has enough attempts; diagnostic should not keep repeating.")];
  }
  if (recommendation.kind === "challenge" && averageTargetScore < 78) {
    return [recommendationIssue(learner, recommendation, targetIds, "challenge_too_early", "watch", "Challenge recommendation appears too hard for current mastery evidence.")];
  }
  if ((recommendation.kind === "practice" || recommendation.kind === "review") && allTargetsStable) {
    return [recommendationIssue(learner, recommendation, targetIds, "repeated_without_progress", "watch", "Recommendation points to already stable targets; move learner forward or mix challenge practice.")];
  }
  return [];
}
...
function auditRecommendationRepetition(
  ...
  if (recentAttempts.length >= 3 && recentAttempts.every((attempt) => !attempt.correct)) {
    return [recommendationIssue(learner, recommendation, targetIds, "repeated_without_progress", "watch", "The same target has repeated recent errors; assign prerequisite repair or teacher intervention before more same-level practice.")];
  }
  return [];
}
...
export function buildLearningEventReadiness(
  ...
  const realTelemetryEvents = telemetryEvents.filter((event) => isLearningEvidenceEvent(event) && !isSeedTelemetryEvent(event)).length;
  const seededTelemetryEvents = telemetryEvents.filter((event) => isLearningEvidenceEvent(event) && isSeedTelemetryEvent(event)).length;
  const stateLearningEvents = stateEvents.length;
  const syntheticStateLearningEvents = syntheticStateEvents.length;
  const status: BetaCheckStatus = realTelemetryEvents > 0 || stateLearningEvents >= 5 ? "pass" : normalized.length > 0 ? "watch" : "blocked";
...
export function buildGraphAdjustmentCandidates(
  ...
  const weakMastery = learners.flatMap((learner) => {
    ...
    return computeMastery(learner.state)
      ...
      .map((row) => masteryToAdjustmentCandidate(row, learner.targetProgramIds[0] || row.domainId, learnerEvidenceSource(learner)));
  });
  ...
  const invalidMetadata = contentUnits
    .flatMap((unit) => [...(unit.conceptIds || []), ...(unit.skillIds || [])].filter((id) => !hasGraphNode(graph, id)).map((id) => ({ unit, id })))
    .slice(0, 12)
    .map(({ unit, id }) => ({
      id,
  ...
  const candidates = learners.flatMap((learner) => {
    ...
    return mastery
      ...
      .flatMap((row) => {
        ...
        const isStuck = row.attempts >= repairEvidenceThreshold || consecutiveWrongAttempts >= consecutiveWrongThreshold;
        if (!isStuck) return [];
        ...
        const suspectedPrerequisiteIds = uniqueStrings(prerequisiteIds.filter((id) => {
...
function buildScopeReport(
  ...
  if (activeContent.length < scope.minContentUnits) blockers.push(`Need ${scope.minContentUnits - activeContent.length} more active content units.`);
  if (questionCount < scope.minQuestions) blockers.push(`Need ${scope.minQuestions - questionCount} more questions.`);
  if (scopedLearners.length < scope.minLearners) nextActions.push("Assign at least one internal beta learner to this program.");
...
function buildRunPlanNextActions(report: BetaReadinessReport, cohorts: InternalBetaCohort[]): string[] {
  ...
  cohorts.forEach((cohort) => {
    if (cohort.currentLearners < cohort.targetLearners) {
      actions.push(`Recruit ${cohort.targetLearners - cohort.currentLearners} more internal learners for ${cohort.title}.`);
    }
    if (cohort.status === "blocked") {
      actions.push(`Clear blockers before launching ${cohort.title}.`);
    }
  });
  if (report.learningEvents.status !== "pass") actions.push("Collect live learning events from diagnostic/practice/review flows.");
  if (report.diagnosticAudit.status !== "pass") actions.push("Run at least one diagnostic attempt per beta learner.");
  if (report.recommendationAudit.status === "blocked") actions.push("Fix recommendation targets before assigning adaptive paths.");
  const liveRerouteCandidates = report.repairRerouteCandidates.filter((candidate) => candidate.evidenceSource !== "synthetic");
  const syntheticRerouteCandidates = report.repairRerouteCandidates.filter((candidate) => candidate.evidenceSource === "synthetic");
  if (liveRerouteCandidates.length) {
    actions.push(`Review ${liveRerouteCandidates.length} live stuck-repair reroute candidate(s) before assigning more same-level practice.`);
  } else if (syntheticRerouteCandidates.length) {
    actions.push(`Track ${syntheticRerouteCandidates.length} simulated stuck-repair reroute candidate(s) separately from live beta evidence.`);
  }
  if (!report.graphAdjustmentCandidates.length) actions.push("Wait for live beta evidence before changing Knowledge Graph edges.");
...
function buildScopeChangeImpactItems(
  ...
  return currentScopes.flatMap((scope) => {
    ...
    const changed = !previous || previous.status !== scope.status || previous.contentUnits !== scope.contentUnits || previous.questionCount !== scope.questionCount || previous.learners !== scope.learners || previous.targetCoverage !== scope.targetCoverage;
    ...
    return [{
      ...
      why: scope.blockers.length ? scope.blockers.join(" ") : scope.nextActions.length ? scope.nextActions.join(" ") : scopeDetail(scope),
      impact: betaStatusImpact(scope.status, "Scope is ready for the internal beta cohort.", "Scope can run, but it needs more learner evidence or coverage review.", "Scope should not be expanded until blockers are cleared."),
...
function buildChecklistChangeImpactItems(
  ...
  return currentItems.flatMap((item) => {
    ...
    const changed = !previous || previous.status !== item.status || previous.evidenceCount !== item.evidenceCount || previous.detail !== item.detail;
    const changeType = !hasPrevious ? "baseline" : !previous ? "added" : changed ? "updated" : "unchanged_risk";
    ...
    return [{
      ...
      why: item.detail,
      impact: betaStatusImpact(item.status, "Gate is acceptable for internal beta.", "Gate needs monitoring before wider rollout.", "Gate blocks rollout until fixed."),
...
function buildLearningKpiChangeImpactItem(
  ...
  return {
    ...
    why: current.detail,
    impact: betaStatusImpact(current.status, "Learning impact is strong enough to trust the current path.", "Learning impact has signals but needs more evidence before graph changes.", "Learning data is not strong enough for cohort expansion or graph changes."),
...
function buildRecommendationQualityChangeImpactItem(
  ...
  return {
    ...
    why: current.detail,
    impact: betaStatusImpact(current.status, "Adaptive recommendations can be assigned.", "Recommendations are usable with teacher/admin monitoring.", "Adaptive recommendations must be fixed before student rollout."),
...
function buildGraphChangeImpactItems(
  ...
  return currentCandidates.slice(0, 12).map((candidate) => {
    ...
    const changed = !previous || previous.evidenceCount !== candidate.evidenceCount || previous.score !== candidate.score || previous.reason !== candidate.reason;
...
function buildRepairRerouteChangeImpactItems(
  ...
  const previousById = new Map(previousCandidates.map((candidate) => [repairRerouteCandidateKey(candidate), candidate]));
  return currentCandidates.slice(0, 8).map((candidate) => {
    ...
    return {
      ...
      why: candidate.reason,
      impact: candidate.evidenceSource === "synthetic"
...
function betaStatusImpact(status: BetaCheckStatus, passImpact: string, watchImpact: string, blockedImpact: string): string {
...
function weeklyCohortNextAction(
  ...
  if (cohort.status === "blocked" || scope?.status === "blocked") return `Clear blockers for ${cohort.title} before adding more learners.`;
  if (cohort.currentLearners < cohort.targetLearners) return `Recruit ${cohort.targetLearners - cohort.currentLearners} learner(s) for ${cohort.title}.`;
  if (evidencePercent < 70) return `Collect more diagnostic/practice/review events for ${cohort.title}.`;
  if (scope?.targetCoverage && scope.targetCoverage < 80) return `Improve graph coverage for ${cohort.title} before wider rollout.`;
...
function buildWeeklyReviewDecisions(
  ...
  if (report.recommendationQualityAudit.status === "blocked") {
    decisions.push("Pause adaptive recommendation assignment for affected learners until recommendation quality issues are fixed.");
  }
  const liveReroutes = report.repairRerouteCandidates.filter((candidate) => candidate.evidenceSource !== "synthetic").length;
  const syntheticReroutes = report.repairRerouteCandidates.filter((candidate) => candidate.evidenceSource === "synthetic").length;
  if (liveReroutes) {
    decisions.push(`${liveReroutes} live stuck-repair reroute candidate(s) need admin/teacher review before more same-level practice.`);
  } else if (syntheticReroutes) {
    decisions.push(`${syntheticReroutes} simulated stuck-repair reroute candidate(s) are watch-only until live beta evidence confirms them.`);
  }
  ...
  if (!graphBacklog.length) decisions.push("No graph changes should be made until live beta evidence creates a backlog item.");
...
function calculateScopeTargetCoverage(graph: KnowledgeGraph, contentUnits: BetaContentUnit[], scope: BetaScopeConfig): number {
  const targets = scopeTargetIds(graph, scope);
  if (!targets.length) return 0;
  const contentTargets = new Set(contentUnits.flatMap((unit) => [...(unit.conceptIds || []), ...(unit.skillIds || [])]));
...
function scopeTargetIds(graph: KnowledgeGraph, scope: BetaScopeConfig): string[] {
  ...
  if (scopedObjectives.length) {
    return uniqueStrings(scopedObjectives.flatMap((objective) => [...(objective?.conceptIds || []), ...(objective?.skillIds || [])]));
  }
...
function masteryToAdjustmentCandidate(row: MasteryEstimate, programId: string, evidenceSource: BetaEvidenceSource): GraphAdjustmentCandidate {
  return {
    id: row.id,
    kind: "weak_mastery",
    programId,
...
function buildMisconceptionSignalCandidates(graph: KnowledgeGraph, learners: BetaLearner[]): GraphAdjustmentCandidate[] {
  const byMisconception = new Map<string, { count: number; programIds: Map<string, number>; conceptIds: Set<string>; skillIds: Set<string>; evidenceSources: Set<BetaEvidenceSource> }>();
...
function misconceptionToAdjustmentCandidate(
  ...
  entry: { count: number; programIds: Map<string, number>; conceptIds: Set<string>; skillIds: Set<string>; evidenceSources: Set<BetaEvidenceSource> },
): GraphAdjustmentCandidate {
