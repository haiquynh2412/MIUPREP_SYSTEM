...
console.log(`Status: ${runPlan.status}; scopes: ${report.scopes.map((scope) => `${scope.scopeId}=${scope.status}`).join(", ")}`);
...
function buildIeltsReadingListeningUnits(items: QuestionItem[]): BetaContentUnit[] {
  ...
  items
    ...
    .forEach((item) => {
      ...
      const skill = String(item.metadata?.testSkill || item.tags.find((tag) => ["reading", "listening"].includes(String(tag).toLowerCase())) || "mock");
...
function getMiuMathQuestionsPath(): string {
  ...
  return provided || path.resolve(__dirname, "..", "..", "..", "apps", "miumath-app", "public", "data", "questions_db.json");
}
  ...
  return [...groups.values()]
    .map((group) => {
      ...
      return {
        ...
        conceptIds: uniqueStrings(group.flatMap((item) => item.conceptIds)).slice(0, 12),
        skillIds: uniqueStrings(group.flatMap((item) => item.skillIds)).slice(0, 12),
        tags: uniqueStrings(["math", "grade9", category, subCategory, ...group.flatMap((item) => item.tags)]).slice(0, 12),
      };
    })
...
function buildSeedBetaLearners(): BetaLearner[] {
  ...
  return [
    { id: "beta-math-learner-1", username: "Internal Math Beta Learner", targetProgramIds: ["vn_math_6_9"], state: mathState, stateKind: "synthetic" },
    { id: "beta-ielts-learner-1", username: "Internal IELTS Beta Learner", targetProgramIds: ["ielts"], state: ieltsState, stateKind: "synthetic" },
  ];
}

function buildSeedTelemetryEvents(now: string): BetaTelemetryEvent[] {
  return [
    { id: "beta-telemetry-math-diagnostic", type: "diagnostic", learnerId: "beta-math-learner-1", programId: "vn_math_6_9", message: "diagnostic attempt submitted", occurredAt: now, source: "beta_export_seed" },
    { id: "beta-telemetry-math-practice", type: "practice", learnerId: "beta-math-learner-1", programId: "vn_math_6_9", message: "practice attempt submitted", occurredAt: now, source: "beta_export_seed" },
    { id: "beta-telemetry-ielts-diagnostic", type: "diagnostic", learnerId: "beta-ielts-learner-1", programId: "ielts", message: "IELTS diagnostic attempt submitted", occurredAt: now, source: "beta_export_seed" },
    { id: "beta-telemetry-ielts-practice", type: "practice", learnerId: "beta-ielts-learner-1", programId: "ielts", message: "IELTS reading practice attempt submitted", occurredAt: now, source: "beta_export_seed" },
  ];
}
...
function renderMarkdown(betaRun: BetaRunExport): string {
  ...
  return [
    ...
    "| --- | --- | --- | --- | --- | --- |",
    ...betaRun.changeImpact.items.map((item) =>
    ...
    "| --- | --- | --- | --- | --- | --- |",
    ...betaRun.weeklyReview.cohortReviews.map((item) =>
    ...
    "| --- | --- | ---: | --- |",
    ...report.checklist.map((item) => `| ${item.label} | ${item.status} | ${item.evidenceCount} | ${escapeMarkdownTable(item.detail)} |`),
    "",
    "## Scopes",
    "",
    "| Scope | Status | Units | Questions | Learners | Coverage |",
    "| --- | --- | ---: | ---: | ---: | ---: |",
    ...report.scopes.map((scope) => `| ${scope.title} | ${scope.status} | ${scope.contentUnits} | ${scope.questionCount} | ${scope.learners} | ${scope.targetCoverage}% |`),
    "",
    ...
    ...(report.graphAdjustmentCandidates.length
      ? report.graphAdjustmentCandidates
          .slice(0, 12)
          .map((candidate) => `- ${candidate.kind}: ${candidate.id} (${candidate.evidenceCount} evidence; source: ${candidate.evidenceSource}; autoApply: no) - ${candidate.reason}`)
...
function renderWeeklyReviewMarkdown(review: WeeklyBetaCohortReview): string {
  return [
    ...
    ...(review.graphBacklog.length
      ? review.graphBacklog.map((item) => `| ${item.id} | ${item.kind} | ${item.severity} | ${item.evidenceCount} | ${item.evidenceSource} | ${item.autoApply ? "yes" : "no"} | ${item.decision} | ${escapeMarkdownTable(item.reason)} |`)
    ...
    ...(review.repairReroutes.length
      ? review.repairReroutes.map((item) => `| ${item.learnerId} | ${item.targetId} | ${item.action} | ${item.evidenceCount} | ${item.consecutiveWrongAttempts} | ${item.evidenceSource} | ${item.decision} | ${escapeMarkdownTable(item.reason)} |`)
