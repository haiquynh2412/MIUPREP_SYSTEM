...
assert(isCorrectAnswer("  30  years old ", [["thirty years old", "30 years old"]]), "Whitespace variation match failed");
...
assert(isCorrectAnswer("iii. Heading 3", headingsQuestion.acceptedAnswers), "Matching headings value mapping failed for full text option");
assert(isCorrectAnswer("iii", headingsQuestion.acceptedAnswers), "Matching headings value mapping failed for raw value option");
assert(!isCorrectAnswer("ii. Heading 2", headingsQuestion.acceptedAnswers), "Matching headings value mapping matched incorrect answer");
...
const xssPayloads = [
  ...
  { input: "Safe <mark class=\"bg-yellow-100\">highlighted</mark> text", expected: "Safe <mark class=\"bg-yellow-100\">highlighted</mark> text" },
...
dbAdapter.initialize().then(() => {
  dbAdapter.getAttempt("attempt-123").then(attempt => {
...
class SimulatedMockAIAdapter {
  async gradeWriting(params) {
    ...
    return {
      ...
      sentenceUpgrades: [{ original: 'o', upgraded: 'u', explanation: 'e', targetedBand: 7.5 }, { original: 'o2', upgraded: 'u2', explanation: 'e2', targetedBand: 8.0 }]
    };
  }
}
...
mockAi.gradeWriting({
  ...
  assert(feedback.bandOverall === 5.5, `Mock AI should penalize short essay: expected 5.5, got ${feedback.bandOverall}`);
...
rubricAi.gradeWriting({
  ...
  assert(taskAchievement.band === 5.5, `Short essay Task Achievement should be capped at 5.5, got ${taskAchievement.band}`);
  assert(Array.isArray(taskAchievement.evidence) && taskAchievement.evidence.length > 0, "Task Achievement must contain evidence quotes");
  assert(typeof taskAchievement.whyNotHigher === 'string' && taskAchievement.whyNotHigher.length > 0, "Task Achievement must explain why not higher in Vietnamese");
  assert(typeof taskAchievement.nextAction === 'string' && taskAchievement.nextAction.length > 0, "Task Achievement must contain actionable next steps in Vietnamese");
...
rubricAi.gradeSpeaking({
  ...
  assert(pron.feedbackText === 'Pronunciation unavailable from transcript', `Pronunciation feedback text mismatch: got "${pron.feedbackText}"`);
...
rubricAi.gradeSpeaking({
  ...
  assert(pron.feedbackText.includes('Generally intelligible'), `Pronunciation feedback text mismatch: got "${pron.feedbackText}"`);
  assert(Array.isArray(pron.evidence) && pron.evidence.length > 0, "Pronunciation must contain evidence");
  assert(typeof pron.whyNotHigher === 'string' && pron.whyNotHigher.length > 0, "Pronunciation must explain why not higher");
