...
Ensure you provide exactly 4 criteria in the array.`;
  ...
  async gradeSpeaking(params: {
    ...
    // 1. Decouple and transcribe audio using Whisper if base64 data is present
    let transcript = params.transcriptMock || '';
    if (params.audioBase64) {
      try {
        console.log('[OpenAI STT] Decoding audio Base64 payload...');
        const binaryStr = atob(params.audioBase64);
        ...
        console.log('[OpenAI STT] Dispatching audio to Whisper transcription service...');
        ...
        if (!whisperRes.ok) {
          ...
          throw new Error(`Whisper STT service returned HTTP ${whisperRes.status}: ${errText}`);
        }
