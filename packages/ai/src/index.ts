...
export { InMemoryCredentialStore, SessionCredentialStore } from './utils/credential-store';
...
export { AIValidationError, validateWritingFeedback, validateSpeakingFeedback } from './utils/schema-validator';
export {
...
export class MockAIAdapter implements AIAdapter {
  async gradeWriting(params: {
    ...
    if (goldenSample) {
      ...
      const criteria: CriteriaScore[] = [
        {
          ...
          band: convertScore(
            params.taskNumber === 1 ? getBand('taskAchievement', overall) : getBand('taskResponse', overall),
          ),
      ...
      return {
        ...
        descriptorSource: isCpeCae
          ? 'Cambridge English C2 Proficiency Band Descriptors'
    ...
    const criteria: CriteriaScore[] = [
      {
        ...
        evidence: isTooShort ? [`Word count: ${wordCount} words`] : ['clearly highlighted', 'fully addresses'],
      ...
      },
      {
        ...
        feedbackText:
          'Paragraphing is logical and cohesive devices are used appropriately, though there is occasional mechanical connectivity.',
        ...
        nextAction:
          'Luyện tập sử dụng các trạng từ liên kết ẩn chìm hoặc cấu trúc phân từ thay thế cho việc lạm dụng từ nối đầu dòng.',
      },
      {
        criterionName: mapName('Lexical Resource'),
        band: convertScore(7.0),
        feedbackText:
          'Good range of vocabulary with some less common items and awareness of style/collocation, though occasional spelling errors occur.',
        ...
        nextAction:
          'Lập sổ tay từ vựng đồng nghĩa cho các chủ đề phổ biến và học cách sử dụng chính xác các collocation học thuật.',
      },
      {
        criterionName: mapName('Grammatical Range and Accuracy'),
        band: convertScore(6.5),
        feedbackText:
          'A mix of simple and complex sentence structures is used. Errors in grammar and punctuation are present but do not impede communication.',
        evidence: ['mix of simple and complex', 'Errors in grammar'],
        whyNotHigher:
          'Mặc dù có nỗ lực viết câu phức, tần suất lỗi ngữ pháp cơ bản (chia động từ, mạo từ) vẫn xuất hiện làm cản trở sự tự nhiên.',
      ...
      sentenceUpgrades: [
        {
          ...
          explanation:
            'Nâng cấp từ "graph show" (lỗi S-V agreement) thành "line graph clearly illustrates" trang trọng, và chuyển "increased rapid" thành cụm danh từ "a rapid upward trajectory" đặc trưng band 7.5+.',
          targetedBand: convertScore(7.5),
        },
        {
          ...
          explanation:
            'Nâng cấp từ vựng thông dụng "studying abroad is good" thành các thuật ngữ học thuật chuyên sâu như "tertiary education overseas" và tính từ "highly advantageous".',
  ...
  async gradeSpeaking(params: {
    ...
    const criteria: CriteriaScore[] = [
      {
        ...
        feedbackText:
          'Speaks at length with minor hesitation or repetition. Able to link sentences cleanly using markers.',
        evidence: ['Speaks at length', 'minor hesitation'],
        whyNotHigher:
          'Vẫn còn một số quãng ngập ngừng (hesitation) khi tìm từ vựng hoặc cấu trúc ngữ pháp phức tạp ở phần Part 2.',
      ...
      },
      {
        ...
        band: convertScore(7.0) as number,
        feedbackText:
          'Has sufficient vocabulary to discuss topics at length. Uses some idiomatic language appropriately.',
        ...
        nextAction:
          'Học cách sử dụng tối thiểu 2-3 idioms tự nhiên liên quan đến chủ đề quen thuộc (Work, Study, Travel).',
      },
      {
        criterionName: mapName('Grammatical Range and Accuracy'),
        ...
        whyNotHigher:
          'Gặp lỗi hệ thống ở các câu phức phức tạp và chia động từ ngôi thứ ba số ít (subject-verb agreement).',
        nextAction:
          'Tập trung luyện tập nói câu đơn và câu phức cơ bản thật chính xác trước khi cố gắng sử dụng các cấu trúc quá phức tạp.',
      },
      {
        criterionName: mapName('Pronunciation'),
    ...
    return {
      ...
      fluencyReview:
        'Good overall flow, speaking pace is natural. Focus on reducing slight hesitation before transition words.',
      socraticHints: [
      ...
      sentenceUpgrades: [
        {
          ...
          upgraded:
            'Undeniably, mastering English is of paramount importance given its status as a global lingua franca.',
          explanation:
            'Thay thế từ vựng đơn giản "very important" bằng cụm từ nâng cao "of paramount importance", và "globally" thành "global lingua franca" để đạt Lexical Resource band 7.5+.',
          targetedBand: convertScore(7.5) as number,
        },
      ],
      modelUsed: 'mock-ai-engine-v1',
