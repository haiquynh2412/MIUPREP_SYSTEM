...
const outputDir = getArgValue('--outDir') || path.resolve(process.cwd(), '..', '..', 'reports', 'content-quality', 'cae-deep-audit');
fs.mkdirSync(outputDir, { recursive: true });

const sourceFilesByTestId = buildSourceFileLookup();
const tests = dedupeEnglishExamTests((Object.values(mocks) as unknown[]).filter(isEnglishExamTest)).filter(isCaeTest);
const issues = [
...
const jsonPath = path.join(outputDir, 'cae-deep-audit.json');
const csvPath = path.join(outputDir, 'cae-deep-audit.csv');
const markdownPath = path.join(outputDir, 'cae-deep-audit.md');
...
function addPresentationIssues(
  ...
  if (typo && !isIntentionalErrorCorrectionText(context, field)) {
...
function isCaeTest(test: EnglishExamTest): boolean {
  return inferExam(test) === 'cae';
}

function inferExam(test: EnglishExamTest): 'cae' | '' {
  const text = `${test.exam || ''} ${test.id || ''} ${test.title || ''}`.toLowerCase();
  if (text.includes('cae') || text.includes('advanced') || text.includes('cam-cae')) return 'cae';
  return '';
}
...
function inferSkill(test: EnglishExamTest, section: EnglishExamSection, question: EnglishExamQuestion): string {
  ...
  if (question.topic?.skill) return String(question.topic.skill);
  if (String(question.category || '').includes('use_of_english')) return 'use_of_english';
  if (String(question.category || '').includes('_reading_')) return 'reading';
  if (String(question.category || '').includes('_listening_')) return 'listening';
  if ((inferExam(test) === 'cae') && (/\bpart\s*[1-4]\b/.test(text) || text.includes('cloze') || text.includes('word formation') || text.includes('key word transformation'))) return 'use_of_english';
...
function shouldRequireSourceEvidence(context: AuditContext, question: EnglishExamQuestion, answerLocation: string): boolean {
  ...
  if (isAnswerKeyEvidence(answerLocation)) return false;
...
function isHtmlBoundaryRepeat(rawValue: string, repeatedWord: string): boolean {
  ...
  return new RegExp(`${word}\\s*(?:</(?:h[1-6]|p|b|strong)>\\s*|<br\\s*/?>\\s*|<p[^>]*>\\s*)+(?:<[^>]+>\\s*)*${word}\\b`, 'i').test(rawValue);
}

function isAnswerKeyEvidence(answerLocation: string): boolean {
  return /^answer-key evidence:/i.test(plainText(answerLocation)) || /\bevidence cue:\s+use this answer-key option\b/i.test(plainText(answerLocation));
}

function isIntentionalErrorCorrectionText(context: AuditContext, field: string): boolean {
  const text = `${context.testTitle} ${context.sectionTitle} ${context.groupId} ${context.questionId}`.toLowerCase();
  return context.skill === 'use_of_english' &&
    (text.includes('error correction') || text.includes('proofreading')) &&
    (field === 'passageHtml' || field === 'explanation' || field === 'answerLocation' || field === 'acceptedAnswers');
}
