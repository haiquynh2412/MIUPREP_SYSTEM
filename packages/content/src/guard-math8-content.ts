import { buildMath8ContentGuardReport, type Math8ContentGuardReport, type Math8ContentIssue } from './math8-content-guard-report';
...
const rawPath = getPathArg('--raw', resolveDefaultMath8RawPath());
...
const hsgPath = path.resolve(workspaceRoot, 'reports/content-quality/math8-hsg-raw-extract.json');
if (fs.existsSync(hsgPath)) {
  const hsgSources = readRawSources(hsgPath);
  rawSources.push(...hsgSources);
}
const { report, items, displayReadyItems } = buildMath8ContentGuardReport(rawSources, { rawPath });

fs.mkdirSync(outputDir, { recursive: true });
fs.writeFileSync(path.join(outputDir, 'math8-content-guard.json'), JSON.stringify(report, null, 2), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math8-content-audit.md'), renderMarkdownReport(report), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math8-content-issues.csv'), renderIssuesCsv(report.issues), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math8-display-ready-preview.json'), JSON.stringify({
  report: {
...
function readRawSources(filePath: string): Math8RawSource[] {
  if (!fs.existsSync(filePath)) {
    throw new Error(`Math 8 raw extract file does not exist: ${filePath}`);
  }
  const parsed = JSON.parse(fs.readFileSync(filePath, 'utf8').replace(/^\uFEFF/, ''));
  if (Array.isArray(parsed)) return parsed as Math8RawSource[];
  if (Array.isArray(parsed.sources)) return parsed.sources as Math8RawSource[];
  throw new Error(`Math 8 raw extract must be an array or { sources: [] }: ${filePath}`);
}

function renderMarkdownReport(report: Math8ContentGuardReport): string {
  const lines = [
    '# Math 8 Content Audit',
    '',
...
function renderIssuesCsv(issues: Math8ContentIssue[]): string {
...
function resolveDefaultMath8RawPath(): string {
  return path.resolve(workspaceRoot, 'reports/content-quality/math8-rich-raw-extract.json');
}
