import { buildMath1ContentGuardReport, type Math1ContentGuardReport, type Math1ContentIssue } from './math1-content-guard-report';
...
const rawPath = getPathArg('--raw', resolveDefaultMath1RawPath());
...
const { report, items, displayReadyItems } = buildMath1ContentGuardReport(rawSources, { rawPath });

fs.mkdirSync(outputDir, { recursive: true });
fs.writeFileSync(path.join(outputDir, 'math1-content-guard.json'), JSON.stringify(report, null, 2), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math1-content-audit.md'), renderMarkdownReport(report), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math1-content-issues.csv'), renderIssuesCsv(report.issues), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math1-display-ready-preview.json'), JSON.stringify({
  report: {
...
function readRawSources(filePath: string): Math1RawSource[] {
  if (!fs.existsSync(filePath)) {
    throw new Error(`Math 1 raw extract file does not exist: ${filePath}`);
  }
  const parsed = JSON.parse(fs.readFileSync(filePath, 'utf8').replace(/^\uFEFF/, ''));
  if (Array.isArray(parsed)) return parsed as Math1RawSource[];
  if (Array.isArray(parsed.sources)) return parsed.sources as Math1RawSource[];
  throw new Error(`Math 1 raw extract must be an array or { sources: [] }: ${filePath}`);
}

function renderMarkdownReport(report: Math1ContentGuardReport): string {
  const lines = [
    '# Math 1 Content Audit',
    '',
...
function renderIssuesCsv(issues: Math1ContentIssue[]): string {
...
function resolveDefaultMath1RawPath(): string {
  return path.resolve(workspaceRoot, 'reports/content-quality/math1-rich-raw-extract.json');
}
