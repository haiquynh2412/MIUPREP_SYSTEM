import { buildMath4ContentGuardReport, type Math4ContentGuardReport, type Math4ContentIssue } from './math4-content-guard-report';
...
const rawPath = getPathArg('--raw', resolveDefaultMath4RawPath());
...
const { report, items, displayReadyItems } = buildMath4ContentGuardReport(rawSources, { rawPath });

fs.mkdirSync(outputDir, { recursive: true });
fs.writeFileSync(path.join(outputDir, 'math4-content-guard.json'), JSON.stringify(report, null, 2), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math4-content-audit.md'), renderMarkdownReport(report), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math4-content-issues.csv'), renderIssuesCsv(report.issues), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math4-display-ready-preview.json'), JSON.stringify({
  report: {
...
function readRawSources(filePath: string): Math4RawSource[] {
  if (!fs.existsSync(filePath)) {
    throw new Error(`Math 4 raw extract file does not exist: ${filePath}`);
  }
  const parsed = JSON.parse(fs.readFileSync(filePath, 'utf8').replace(/^\uFEFF/, ''));
  if (Array.isArray(parsed)) return parsed as Math4RawSource[];
  if (Array.isArray(parsed.sources)) return parsed.sources as Math4RawSource[];
  throw new Error(`Math 4 raw extract must be an array or { sources: [] }: ${filePath}`);
}

function renderMarkdownReport(report: Math4ContentGuardReport): string {
  const lines = [
...
function renderIssuesCsv(issues: Math4ContentIssue[]): string {
...
function resolveDefaultMath4RawPath(): string {
  return path.resolve(workspaceRoot, 'reports/content-quality/math4-rich-raw-extract.json');
}
