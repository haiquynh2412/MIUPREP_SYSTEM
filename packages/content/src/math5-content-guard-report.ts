import { buildMath5QuestionItemsFromRawSources, type Math5RawSource } from './math5-import';
import type { QuestionItem } from './standard';
import { validateStandardContentBundle, type ValidationError } from './validator';

export type Math5ContentIssueSeverity = 'blocker' | 'warning';

export interface Math5ContentIssue {
  code: string;
  severity: Math5ContentIssueSeverity;
  message: string;
  path: string;
  questionId?: string;
  sourceFile?: string;
...
export interface Math5ContentGuardReport {
  schemaVersion: 'math5_content_guard_v1';
  generatedAt: string;
  input: {
    rawPath?: string;
    sources: number;
  ...
  issues: Math5ContentIssue[];
  displayReadyItemIds: string[];
  scoredPracticeReadyItemIds: string[];
}

export interface BuildMath5ContentGuardReportOptions {
  rawPath?: string;
  generatedAt?: string;
}

export function buildMath5ContentGuardReport(
  rawSources: Math5RawSource[],
  options: BuildMath5ContentGuardReportOptions = {},
): { report: Math5ContentGuardReport; items: QuestionItem[]; displayReadyItems: QuestionItem[] } {
  const importResult = buildMath5QuestionItemsFromRawSources(rawSources);
  const adapterErrors = validateStandardContentBundle({ questions: importResult.items });

  const issues: Math5ContentIssue[] = [];
  ...
  const report: Math5ContentGuardReport = {
    schemaVersion: 'math5_content_guard_v1',
...
function summarizeIssues(questionCount: number, issues: Math5ContentIssue[]): Math5ContentGuardReport['qualitySummary'] {
