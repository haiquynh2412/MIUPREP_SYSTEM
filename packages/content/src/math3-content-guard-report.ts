import { buildMath3QuestionItemsFromRawSources, type Math3RawSource } from './math3-import';
import type { QuestionItem } from './standard';
import { validateStandardContentBundle, type ValidationError } from './validator';

export type Math3ContentIssueSeverity = 'blocker' | 'warning';

export interface Math3ContentIssue {
  code: string;
  severity: Math3ContentIssueSeverity;
  message: string;
  path: string;
  questionId?: string;
  sourceFile?: string;
...
export interface Math3ContentGuardReport {
  schemaVersion: 'math3_content_guard_v1';
  generatedAt: string;
  input: {
    rawPath?: string;
    sources: number;
  ...
  issues: Math3ContentIssue[];
  displayReadyItemIds: string[];
  scoredPracticeReadyItemIds: string[];
}

export interface BuildMath3ContentGuardReportOptions {
  rawPath?: string;
  generatedAt?: string;
}

export function buildMath3ContentGuardReport(
  rawSources: Math3RawSource[],
  options: BuildMath3ContentGuardReportOptions = {},
): { report: Math3ContentGuardReport; items: QuestionItem[]; displayReadyItems: QuestionItem[] } {
  const importResult = buildMath3QuestionItemsFromRawSources(rawSources);
  const adapterErrors = validateStandardContentBundle({ questions: importResult.items });

  const issues: Math3ContentIssue[] = [];
  ...
  const report: Math3ContentGuardReport = {
    schemaVersion: 'math3_content_guard_v1',
...
function summarizeIssues(questionCount: number, issues: Math3ContentIssue[]): Math3ContentGuardReport['qualitySummary'] {
