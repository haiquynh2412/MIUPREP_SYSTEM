import { buildMath2ContentGuardReport, type Math2ContentGuardReport, type Math2ContentIssue } from './math2-content-guard-report';
...
const rawPath = getPathArg('--raw', resolveDefaultMath2RawPath());
...
const { report, items, displayReadyItems } = buildMath2ContentGuardReport(rawSources, { rawPath });

fs.mkdirSync(outputDir, { recursive: true });
fs.writeFileSync(path.join(outputDir, 'math2-content-guard.json'), JSON.stringify(report, null, 2), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math2-content-audit.md'), renderMarkdownReport(report), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math2-content-issues.csv'), renderIssuesCsv(report.issues), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math2-display-ready-preview.json'), JSON.stringify({
  report: {
...
function readRawSources(filePath: string): Math2RawSource[] {
  if (!fs.existsSync(filePath)) {
    throw new Error(`Math 2 raw extract file does not exist: ${filePath}`);
  }
  const parsed = JSON.parse(fs.readFileSync(filePath, 'utf8').replace(/^\uFEFF/, ''));
  if (Array.isArray(parsed)) return parsed as Math2RawSource[];
  if (Array.isArray(parsed.sources)) return parsed.sources as Math2RawSource[];
  throw new Error(`Math 2 raw extract must be an array or { sources: [] }: ${filePath}`);
}

function renderMarkdownReport(report: Math2ContentGuardReport): string {
  const lines = [
    '# Math 2 Content Audit',
    '',
...
function renderIssuesCsv(issues: Math2ContentIssue[]): string {
...
function resolveDefaultMath2RawPath(): string {
  return path.resolve(workspaceRoot, 'reports/content-quality/math2-rich-raw-extract.json');
}
