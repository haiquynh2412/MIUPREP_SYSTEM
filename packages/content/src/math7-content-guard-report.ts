import { buildMath7QuestionItemsFromRawSources, type Math7RawSource } from './math7-import';
import type { QuestionItem } from './standard';
import { validateStandardContentBundle, type ValidationError } from './validator';

export type Math7ContentIssueSeverity = 'blocker' | 'warning';

export interface Math7ContentIssue {
  code: string;
  severity: Math7ContentIssueSeverity;
  message: string;
  path: string;
  questionId?: string;
  sourceFile?: string;
...
export interface Math7ContentGuardReport {
  schemaVersion: 'math7_content_guard_v1';
  generatedAt: string;
  input: {
    rawPath?: string;
    sources: number;
  ...
  issues: Math7ContentIssue[];
  displayReadyItemIds: string[];
}

export interface BuildMath7ContentGuardReportOptions {
  rawPath?: string;
  generatedAt?: string;
}
...
export function buildMath7ContentGuardReport(
  rawSources: Math7RawSource[],
  options: BuildMath7ContentGuardReportOptions = {},
): { report: Math7ContentGuardReport; items: QuestionItem[]; displayReadyItems: QuestionItem[] } {
  const importResult = buildMath7QuestionItemsFromRawSources(rawSources);
  ...
  const issues: Math7ContentIssue[] = [];
  ...
  const report: Math7ContentGuardReport = {
    schemaVersion: 'math7_content_guard_v1',
...
function getRawOleMarkerCount(source: Math7RawSource): number {
  return typeof source.rawOleMarkerCount === 'number' ? source.rawOleMarkerCount : countMatches(source.text, /\u0001/g);
}

function summarizeIssues(questionCount: number, issues: Math7ContentIssue[]): Math7ContentGuardReport['qualitySummary'] {
  return {
