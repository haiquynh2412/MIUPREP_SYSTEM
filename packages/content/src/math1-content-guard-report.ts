import { buildMath1QuestionItemsFromRawSources, type Math1RawSource } from './math1-import';
import type { QuestionItem } from './standard';
import { validateStandardContentBundle, type ValidationError } from './validator';

export type Math1ContentIssueSeverity = 'blocker' | 'warning';

export interface Math1ContentIssue {
  code: string;
  severity: Math1ContentIssueSeverity;
  message: string;
  path: string;
  questionId?: string;
  sourceFile?: string;
...
export interface Math1ContentGuardReport {
  schemaVersion: 'math1_content_guard_v1';
  generatedAt: string;
  input: {
    rawPath?: string;
    sources: number;
  ...
  issues: Math1ContentIssue[];
  displayReadyItemIds: string[];
  scoredPracticeReadyItemIds: string[];
}

export interface BuildMath1ContentGuardReportOptions {
  rawPath?: string;
  generatedAt?: string;
}

export function buildMath1ContentGuardReport(
  rawSources: Math1RawSource[],
  options: BuildMath1ContentGuardReportOptions = {},
): { report: Math1ContentGuardReport; items: QuestionItem[]; displayReadyItems: QuestionItem[] } {
  const importResult = buildMath1QuestionItemsFromRawSources(rawSources);
  const adapterErrors = validateStandardContentBundle({ questions: importResult.items });

  const issues: Math1ContentIssue[] = [];
  ...
  const report: Math1ContentGuardReport = {
    schemaVersion: 'math1_content_guard_v1',
...
function summarizeIssues(questionCount: number, issues: Math1ContentIssue[]): Math1ContentGuardReport['qualitySummary'] {
