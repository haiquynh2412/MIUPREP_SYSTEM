import { buildMath2QuestionItemsFromRawSources, type Math2RawSource } from './math2-import';
import type { QuestionItem } from './standard';
import { validateStandardContentBundle, type ValidationError } from './validator';

export type Math2ContentIssueSeverity = 'blocker' | 'warning';

export interface Math2ContentIssue {
  code: string;
  severity: Math2ContentIssueSeverity;
  message: string;
  path: string;
  questionId?: string;
  sourceFile?: string;
...
export interface Math2ContentGuardReport {
  schemaVersion: 'math2_content_guard_v1';
  generatedAt: string;
  input: {
    rawPath?: string;
    sources: number;
  ...
  issues: Math2ContentIssue[];
  displayReadyItemIds: string[];
  scoredPracticeReadyItemIds: string[];
}

export interface BuildMath2ContentGuardReportOptions {
  rawPath?: string;
  generatedAt?: string;
}

export function buildMath2ContentGuardReport(
  rawSources: Math2RawSource[],
  options: BuildMath2ContentGuardReportOptions = {},
): { report: Math2ContentGuardReport; items: QuestionItem[]; displayReadyItems: QuestionItem[] } {
  const importResult = buildMath2QuestionItemsFromRawSources(rawSources);
  const adapterErrors = validateStandardContentBundle({ questions: importResult.items });

  const issues: Math2ContentIssue[] = [];
  ...
  const report: Math2ContentGuardReport = {
    schemaVersion: 'math2_content_guard_v1',
...
function summarizeIssues(questionCount: number, issues: Math2ContentIssue[]): Math2ContentGuardReport['qualitySummary'] {
