import { buildMath6ContentGuardReport, type Math6ContentGuardReport, type Math6ContentIssue } from './math6-content-guard-report';
...
fs.writeFileSync(path.join(outputDir, 'math6-content-audit.md'), renderMarkdownReport(report), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math6-content-issues.csv'), renderIssuesCsv(report.issues), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math6-display-ready-preview.json'), JSON.stringify({
  report: {
...
function renderMarkdownReport(report: Math6ContentGuardReport): string {
  const lines = [
...
function renderIssuesCsv(issues: Math6ContentIssue[]): string {
