import { buildMath3ContentGuardReport, type Math3ContentGuardReport, type Math3ContentIssue } from './math3-content-guard-report';
...
const rawPath = getPathArg('--raw', resolveDefaultMath3RawPath());
...
const { report, items, displayReadyItems } = buildMath3ContentGuardReport(rawSources, { rawPath });

fs.mkdirSync(outputDir, { recursive: true });
fs.writeFileSync(path.join(outputDir, 'math3-content-guard.json'), JSON.stringify(report, null, 2), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math3-content-audit.md'), renderMarkdownReport(report), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math3-content-issues.csv'), renderIssuesCsv(report.issues), 'utf8');
fs.writeFileSync(path.join(outputDir, 'math3-display-ready-preview.json'), JSON.stringify({
  report: {
...
function readRawSources(filePath: string): Math3RawSource[] {
  if (!fs.existsSync(filePath)) {
    throw new Error(`Math 3 raw extract file does not exist: ${filePath}`);
  }
  const parsed = JSON.parse(fs.readFileSync(filePath, 'utf8').replace(/^\uFEFF/, ''));
  if (Array.isArray(parsed)) return parsed as Math3RawSource[];
  if (Array.isArray(parsed.sources)) return parsed.sources as Math3RawSource[];
  throw new Error(`Math 3 raw extract must be an array or { sources: [] }: ${filePath}`);
}

function renderMarkdownReport(report: Math3ContentGuardReport): string {
  const lines = [
    '# Math 3 Content Audit',
    '',
...
function renderIssuesCsv(issues: Math3ContentIssue[]): string {
...
function resolveDefaultMath3RawPath(): string {
  return path.resolve(workspaceRoot, 'reports/content-quality/math3-rich-raw-extract.json');
}
