...
export const MATH_ERROR_TAXONOMY: ErrorTaxonomyEntry[] = [
  {
    ...
    name: 'Wrong formula',
    description: 'The learner selected or recalled the wrong formula.',
  },
  {
    id: 'missing_condition',
    domainId: 'mathematics',
    name: 'Missing condition',
    description: 'The learner ignored a required condition or constraint.',
  },
  {
    id: 'algebra_transform',
    domainId: 'mathematics',
    name: 'Algebra transform',
    description: 'The learner made an invalid transformation step.',
  },
  {
    id: 'calculation',
    domainId: 'mathematics',
    name: 'Calculation',
    description: 'The reasoning path is right but arithmetic or computation is wrong.',
  },
  {
    id: 'reading_prompt',
    domainId: 'mathematics',
    name: 'Reading prompt',
    description: 'The learner misunderstood what the problem asked.',
  },
];
...
export const ENGLISH_ERROR_TAXONOMY: ErrorTaxonomyEntry[] = [
  {
    ...
    name: 'Vocabulary',
    description: 'The learner missed the meaning or register of a word or phrase.',
  },
  {
    id: 'grammar',
    domainId: 'english_core',
    name: 'Grammar',
    description: 'The learner made a grammar or sentence control error.',
  },
  {
    id: 'inference',
    domainId: 'english_core',
    name: 'Inference',
    description: 'The learner failed to infer implicit meaning from evidence.',
  },
  {
    id: 'collocation',
    domainId: 'english_core',
    name: 'Collocation',
    description: 'The learner used or interpreted an unnatural word pairing.',
  },
  {
    id: 'time_management',
    domainId: 'english_core',
    name: 'Time management',
    description: 'The learner likely lost accuracy due to pacing.',
  },
];
...
export function makeAttempt(
  input: Omit<AttemptRecord, 'schemaVersion' | 'id' | 'answeredAt'> & Partial<Pick<AttemptRecord, 'id' | 'answeredAt'>>,
): AttemptRecord {
...
export function recordAttempt(
  state: StudentModel,
  attemptInput: Parameters<typeof makeAttempt>[0],
...
): { state: StudentModel; event: LearningEventRecord } {
...
export function inferMisconceptionIds(input: {
  ...
  const categories = normalizeErrorCategories(input.errorCategories || ['unknown']).filter(
  ...
  const scored = ERROR_CATEGORY_MISCONCEPTION_BRIDGE.filter((entry) => categorySet.has(entry.errorCategory))
...
export function normalizeErrorNotebookEntry(entry: ErrorNotebookEntryCore): ErrorNotebookEntryCore {
  ...
  return {
    ...
    schemaVersion: 'error_notebook_v1',
    id: String(
      entry.id || `err-${stableChecksum([entry.userId, entry.attemptId, entry.questionId, createdAt].join('|'))}`,
    ),
...
export function scheduleErrorNotebookReview(
  entry: ErrorNotebookEntryCore,
  grade: number,
  ...
  if (boundedGrade >= 3) {
    ...
    intervalDays = tuneSrsIntervalDays(
      baseIntervalDays,
      boundedGrade,
...
function tuneSrsIntervalDays(
  ...
  return Math.max(
    1,
    Math.round(baseIntervalDays * difficultyMultiplier * gradeMultiplier * lapseMultiplier * misconceptionMultiplier),
  );
}
...
export function getDueErrorNotebookEntries(
...
export function summarizeErrorNotebook(
  entries: ErrorNotebookEntryCore[],
  now: string = new Date().toISOString(),
...
export function buildLearningPath(
  ...
  const steps = orderedIds.map((id, index) =>
    buildLearningPathStep(nodeById.get(id)!, masteryById, incoming, targetSet, index),
  );
  ...
  const nextStep =
    trimmedSteps.find((step) => step.status !== 'stable') || trimmedSteps[trimmedSteps.length - 1] || null;
...
export function buildAttemptLearningEvent(
  attempt: AttemptRecord,
  source = 'miuprep_learning_core',
): LearningEventRecord {
  return buildLearningEvent(
    `${attempt.mode}_attempt`,
    {
      attemptId: attempt.id,
  ...
  for (const event of incoming) {
    ...
    if (sameIdempotency) {
      if (sameIdempotency.payloadHash !== event.payloadHash) {
        conflicts.push(
          buildLearningEventMergeConflict('same_idempotency_key_different_payload', sameIdempotency, event),
        );
      } else {
        duplicates += 1;
      }
...
export function buildLearningEventSyncAuditReport(
  ...
  const normalizedWithRepeats = (Array.isArray(events) ? events : []).flatMap((event) =>
    normalizeLearningEvents([event]),
  );
  ...
  const missingEntityIds = normalized.filter(
    (event) => !event.entityId && !event.payload?.itemId && !event.payload?.attemptId,
  ).length;
  const invalidTimestamps = normalized.filter(
    (event) => !isValidIsoLike(event.occurredAt) || !isValidIsoLike(event.receivedAt),
  ).length;
  const feedbackOnlyEvents = normalized.filter(
    (event) => event.type === 'feedback_only' || event.payload?.masteryPolicy === 'feedback_only',
  ).length;
  const conflicts = options.conflicts || [];
  const status: LearningEventSyncAuditReport['status'] =
    conflicts.length || missingLearnerIds || missingEntityIds || invalidTimestamps
      ? 'blocked'
...
export function buildStudentModelFromLearningEvents(
  ...
  const scopedEvents =
    learnerId === 'unknown_learner' ? normalized : normalized.filter((event) => event.learnerId === learnerId);
  ...
  const feedbackOnlyEvents = scopedEvents.filter(
    (event) => event.type === 'feedback_only' || event.payload?.masteryPolicy === 'feedback_only',
  ).length;
  const updatedAt =
    attempts[attempts.length - 1]?.answeredAt || scopedEvents[scopedEvents.length - 1]?.occurredAt || generatedAt;
...
export function saveLearningEventToStorage(
  event: LearningEventRecord | null | undefined,
  storage?: LearningEventStorageLike | null,
): LearningEventRecord | null {
  return saveLearningEventsToStorage(event ? [event] : [], storage)[0] || null;
}

export function saveLearningEventsToStorage(
  events: unknown[] = [],
  storage?: LearningEventStorageLike | null,
): LearningEventRecord[] {
...
export function listLearningEventsFromStorage(
  storage?: LearningEventStorageLike | null,
  options: ListLearningEventsFromStorageOptions = {},
): LearningEventRecord[] {
  const targetStorage = resolveLearningEventStorage(storage);
  if (!targetStorage) return [];
  ...
  const normalized = normalizeLearningEvents(
    ids.map((id) => safeJsonParse(targetStorage.getItem(sharedLearningEventStorageKey(id)), null)).filter(Boolean),
  );
...
export function computeMasteryV2ShadowReport(
  input: StudentModel | AttemptRecord[],
  options: MasteryV2ShadowOptions = {},
  ...
  const protectedFeedbackOnlyEvents = Array.isArray(input)
    ? 0
    : normalizeLearningEvents(input.learningEvents || []).filter(
        (event) => event.payload?.masteryPolicy === 'feedback_only' || event.type === 'feedback_only',
      ).length;
  const rows = v1Rows
    ...
    .sort(
      (a, b) =>
        ...
        statusRank(a.v2Status) - statusRank(b.v2Status) ||
...
export function buildEmpiricalDifficultyShadowReport(
  ...
  const rows = [...attemptsByItem.entries()]
    .map(([itemId, itemAttempts]) =>
      buildEmpiricalDifficultyRow(itemId, itemAttempts, learnerAccuracy, minAttempts, driftThreshold),
    )
    .sort(
      (a, b) =>
        Number(a.sparse) - Number(b.sparse) ||
        Math.abs(b.eloDelta) - Math.abs(a.eloDelta) ||
        a.itemId.localeCompare(b.itemId),
    );
  const sparseItems = rows.filter((row) => row.sparse).length;
  const driftWatchItems = rows.filter(
    (row) => !row.sparse && difficultyDistance(row.priorDifficulty, row.empiricalDifficulty) >= driftThreshold,
  ).length;
  const status: EmpiricalDifficultyShadowReport['status'] =
    rows.length === 0 ? 'watch' : rows.every((row) => row.sparse) ? 'watch' : 'pass';
...
export function recommendNextAction(
  state: StudentModel,
  options: { diagnosticMinAttempts?: number; rerouteFailureThreshold?: number } = {},
): Recommendation {
...
function buildRerouteRecommendation(row: MasteryEstimate, threshold: number): Recommendation {
  ...
  const scopedIds =
    row.scope === 'concept' ? { conceptIds: [row.id], skillIds: [] } : { conceptIds: [], skillIds: [row.id] };
...
export function buildDiagnosticSet<T extends LearningItem>(
  items: T[],
  attempts: AttemptRecord[] = [],
...
export function buildPracticeSet<T extends LearningItem>(
  items: T[],
  attempts: AttemptRecord[] = [],
  ...
  const pool = items.filter((item) => {
    if (attempted.has(item.id)) return false;
    if (options.domainId && item.domainId !== options.domainId) return false;
    if (options.difficulty && normalizeDifficulty(item.difficulty || '') !== normalizeDifficulty(options.difficulty))
      return false;
...
function pickLearningPathTargets(
  ...
  const masteryTargets = mastery
    .filter(
      (row) =>
        (!options.domainId || row.domainId === options.domainId) && nodeById.has(row.id) && row.status !== 'stable',
    )
    .sort((a, b) => statusRank(a.status) - statusRank(b.status) || a.score - b.score || b.attempts - a.attempts)
    .map((row) => row.id);
...
function compareLearningPathEdges(left: LearningPathEdge, right: LearningPathEdge): number {
  ...
  return (
    (typeRank[left.type] ?? 9) - (typeRank[right.type] ?? 9) ||
...
function learningPathStepReason(status: LearningPathStepStatus, isTarget: boolean, prerequisiteCount: number): string {
  ...
  if (status === 'not_started')
    return prerequisiteCount
      ? 'Start here because it unlocks downstream topics.'
      : 'No learning evidence has been collected yet.';
  if (status === 'hard_proof') return 'Add medium or hard problems to prove retention.';
  if (status === 'stable')
    return isTarget ? 'Target area is stable; move into mixed challenge practice.' : 'Prerequisite appears stable.';
...
function readBoolean(value: unknown): boolean | null {
  ...
  if (normalized === 'false' || normalized === 'incorrect' || normalized === 'wrong' || normalized === 'no')
    return false;
  return null;
}
...
function attemptsForMasteryRow(attempts: AttemptRecord[], row: MasteryEstimate): AttemptRecord[] {
  return attempts.filter((attempt) => {
    ...
    const fallbackId =
      row.scope === 'concept' ? `${attempt.domainId}.untagged_concept` : `${attempt.domainId}.untagged_skill`;
...
function buildEmpiricalDifficultyRow(
  ...
  const averageTimeSeconds = Math.round(
    averageNumber(attempts.map((attempt) => Number(attempt.timeSpentSeconds || 0)).filter((value) => value > 0)),
  );
  const hintRate = Math.round(
    (attempts.filter((attempt) => attempt.hintUsed).length / Math.max(1, attempts.length)) * 100,
  );
  const reviewRecurrence = Math.round(
    (attempts.filter((attempt) => attempt.mode === 'review' || !attempt.correct).length /
  ...
  const empiricalDifficulty = sparse
    ? normalizeDifficulty(first.difficulty || 'medium')
    : empiricalDifficultyFromRate(correctRate, hintRate, reviewRecurrence);
...
function buildLearnerAccuracyMap(attempts: AttemptRecord[]): Map<string, number> {
  ...
  return new Map(
    [...byLearner.entries()].map(([learnerId, rows]) => [
      learnerId,
...
function buildMasteryV2ShadowRow(v1: MasteryEstimate, attempts: AttemptRecord[], now: string): MasteryV2ShadowRow {
  ...
  for (const attempt of attempts) {
    const weight =
      attemptModeWeight(attempt.mode) *
      attemptDifficultyWeight(attempt.difficulty || 'medium') *
      attemptRecencyWeight(attempt.answeredAt, now);
    totalWeight += weight;
    if (attempt.correct) {
  ...
  const misconceptionPenalty = repeatedSignalPenalty(
    attempts.flatMap((attempt) => attempt.misconceptionIds || []),
    12,
  );
  ...
  const hintPenalty = Math.round(
    (attempts.filter((attempt) => attempt.hintUsed).length / Math.max(1, attempts.length)) * 8,
  );
  ...
  const v2Score = clamp(
    Math.round(
      weightedAccuracy * evidenceConfidence +
        hardProofBonus -
        recentWrongPenalty -
  ...
  if (attempts.some((attempt) => attempt.correct && ['review', 'mock_test'].includes(String(attempt.mode))))
    strongestEvidence.push('Review/mock evidence supports retention.');
  if (lastCorrectAt && lastCorrectAt >= lastWrongAt)
    strongestEvidence.push('Latest successful evidence is not contradicted by a newer wrong attempt.');
  if (recentWrongPenalty) weakestEvidence.push('Most recent evidence is wrong or unresolved.');
  if (misconceptionPenalty)
    weakestEvidence.push(`Repeated misconceptions applied a ${misconceptionPenalty}-point penalty.`);
...
function updateMastery(
  rows: Map<string, MasteryAccumulator>,
  scope: MasteryScope,
  id: string,
  attempt: AttemptRecord,
): void {
  ...
  row.errorCategories = uniqueStrings([
    ...row.errorCategories,
    ...(attempt.errorCategories || []).filter((category) => category !== 'none'),
  ]) as ErrorCategory[];
...
function masteryStatus(
  row: Pick<MasteryEstimate, 'attempts' | 'accuracy' | 'hardCorrect' | 'lastCorrectAt' | 'lastWrongAt' | 'score'>,
): MasteryStatus {
...
function buildLearningEventMergeConflict(
  ...
  return {
    conflictKey:
      reason === 'same_event_id_different_payload' ? kept.eventId || kept.id : kept.idempotencyKey || kept.id,
    reason,
...
function normalizeEventType(type: string): LearningEventRecord['type'] {
  const normalized = String(type || '')
    ...
    .toLowerCase()
    .replace(/\s+/g, '_');
  if (normalized === 'diagnostic_attempt' || normalized === 'practice_attempt' || normalized === 'review_attempt')
    return normalized;
  if (normalized === 'question_attempt' || normalized === 'mock_test_attempt' || normalized === 'lesson_attempt')
