...
import { IeltsAudioPlayer, SplitScreenReading, sanitizeHtml } from '@miuprep/ui';
    ...
    <div className="flex flex-col gap-6">
      ...
      {isReviewMode && reviewAttempt && (
        <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-md flex flex-col gap-4">
          <h3 className="text-base font-bold text-slate-800 flex items-center gap-2 border-b pb-2">
            <svg className="w-5 h-5 text-accentdeep-600 fill-current" viewBox="0 0 24 24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {analyzeWeaknesses(userAnswers, selectedTest).map((weak) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center justify-between bg-slate-50 border p-3 rounded-lg text-xs text-left"
                    >
                      <div className="flex flex-col text-left">
                        <span className="font-bold text-slate-800">{formatTypeName(weak.questionType)}</span>
                        <span className="text-[10px] text-slate-500 mt-0.5">
                          Correct: {weak.correct}/{weak.total} questions
                        </span>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className={`text-[9px] font-bold px-1.5 py-0.5 border rounded uppercase ${getStatusStyle(weak.status)}`}
                        >
            ...
            <div className="bg-accentdeep-50/50 border border-accentdeep-100 rounded-lg p-5 flex flex-col justify-between text-left">
              <div>
                <h4 className="text-xs font-bold text-accentdeep-900 uppercase tracking-wide mb-2">
                  Lời khuyên sư phạm cá nhân hóa
                </h4>
                  ...
                  {analyzeWeaknesses(userAnswers, selectedTest).map((weak) => {
                    if (weak.status === 'critical') {
                      if (weak.questionType === 'true_false_not_given') {
                        return (
                          <p key={weak.questionType}>
                            ⚠️ <strong>T/F/NG:</strong> Bạn đang yếu dạng phân loại dữ liệu này. Hãy chọn{' '}
                            <strong>FALSE</strong> khi bài đọc trái ngược hoàn toàn; chọn <strong>NOT GIVEN</strong> khi
                            thông tin không xuất hiện.
                          </p>
                        );
                      }
                      if (weak.questionType === 'gap_fill') {
                        return (
                          <p key={weak.questionType}>
                            ⚠️ <strong>Gap Fill (Điền từ):</strong> Cần cẩn thận về giới hạn từ (Word Limit) và dạng từ
                            (danh từ số nhiều/số ít, động từ chia). Luôn kiểm tra ngữ pháp xung quanh ô trống.
                          </p>
                        );
                      }
                      if (weak.questionType === 'multiple_choice') {
                        return (
                          <p key={weak.questionType}>
                            ⚠️ <strong>Multiple Choice:</strong> Cần cảnh giác với phương án gây nhiễu. Hãy tìm sự
                            paraphrase đồng nghĩa thay vì chọn từ giống hệt trong bài đọc.
                          </p>
                        );
                      }
                      return (
                        <p key={weak.questionType}>
                          ⚠️ <strong>{weak.questionType}:</strong> Hãy tập trung làm thêm các bài luyện dạng này và phân
                          tích từ vựng đồng nghĩa.
                        </p>
                      );
                    }
                    if (weak.status === 'needs_improvement') {
                      return (
                        <p key={weak.questionType}>
                          💡 <strong>{weak.questionType}:</strong> Độ chính xác tạm ổn, hãy chú ý thời gian làm bài dạng
                          này để tăng tốc độ.
                        </p>
                      );
                    }
                    return (
                      <p key={weak.questionType}>
                        ✨ <strong>{weak.questionType}:</strong> Kỹ năng làm bài rất tốt! Duy trì phong độ.
                      </p>
                    );
                  })}
                    ...
                    <div className="bg-emerald-50/40 border border-emerald-100/80 rounded-xl p-5 mb-5 flex flex-col gap-3">
                      ...
                      <div className="grid grid-cols-1 gap-4 max-h-[350px] overflow-y-auto pr-1">
                        {(grp as any).paragraphOptions.map((opt: any) => (
                          <div
                            key={opt.label}
                            className="bg-white border border-slate-100 p-3.5 rounded-lg text-xs leading-relaxed text-slate-700 shadow-sm flex gap-3 text-left"
                          >
                                      ...
                                      userAnswers[q.id] === char
                                        ? isReviewMode
                                          ? isCorrect
                                            ...
                                            : 'bg-red-100/70 border-red-300 font-bold'
                            ...
                            <div className="mt-3 p-3 bg-white border border-slate-200 rounded-md text-xs leading-normal flex flex-col gap-1.5 shadow-sm text-slate-700">
                              ...
                              {q.explanation && (
                                <div className="mt-1.5 border-t border-slate-100 pt-1.5">
                                  <strong className="text-accentdeep-900 font-semibold block mb-0.5">
                                    Sư phạm giải thích chi tiết:
                                  </strong>
                              ...
                              {q.answerLocation && (
                                <div className="mt-1.5 border-t border-slate-100 pt-1.5">
                                  <strong className="text-slate-500 font-semibold block mb-0.5">
                                    Dẫn chứng trực tiếp từ bài đọc:
                                  </strong>
      ...
      {selectedTest.skill === 'listening' &&
        selectedTest.sections.map((sec) => (
          <div
            key={sec.id}
            className="flex flex-col gap-6 bg-white border border-slate-200 rounded-lg p-6 shadow-sm text-left"
          >
            <IeltsAudioPlayer
              src={sec.audioPath || ''}
              checksum={sec.audioChecksum}
              examMode={currentExamMode === 'exam'}
            />
  ...
  return (
    <div className="flex flex-col gap-4 text-left">
      ...
      <span className="text-[10px] text-slate-400 italic text-left">
        *Mẹo sư phạm: Click vào từng đoạn hội thoại để AI đọc phát âm chuẩn học thuật (UK/US English), giúp phân tích
        sâu lý do làm sai.
      </span>
    </div>
  );
