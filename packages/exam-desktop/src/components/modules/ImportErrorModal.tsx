...
export default function ImportErrorModal({ importErrors, onClose }: ImportErrorModalProps) {
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col max-h-[80vh]">
        <header className="bg-red-600 text-white px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold tracking-tight m-0 text-white leading-none">
              Exam Package Conformance Failure
            </h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex flex-col gap-4">
          <p className="text-sm text-slate-600 m-0 leading-normal">
            The imported JSON file did not pass the standard {track.label}-style schema validation rules. Please review
            and correct the following errors before re-importing:
          </p>
            ...
            {importErrors.map((err, idx) => (
              ...
              >
                <div className="flex items-center justify-between font-bold">
                  <span>
                    Path: <code className="bg-white/60 px-1 py-0.5 rounded font-mono text-[10px]">{err.path}</code>
                  </span>
