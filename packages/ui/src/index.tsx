...
export const ExamTimer: React.FC<ExamTimerProps> = ({ remainingSeconds, onTimeUp, isPaused = false }) => {
    ...
    <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 flex flex-col gap-3 w-full max-w-xl shadow-sm">
      ...
      {examMode && playCount >= 1 && (
        <p className="text-[10px] text-red-700 bg-red-50 p-2 rounded border border-red-100 font-semibold leading-relaxed">
          {'\u26A0'} CHẾ ĐỘ THI THẬT: Bạn không thể tua âm thanh phát đề thi Listening, chỉ được nghe duy nhất 1 lần và
          không được tạm dừng hay tua lại.
        </p>
      )}
    </div>
  );
...
export const SplitScreenReading: React.FC<SplitScreenReadingProps> = ({ passageHtml, children }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row border border-slate-200 rounded-lg overflow-hidden h-[80vh] bg-white shadow-sm">
      ...
      <div className="w-full md:w-1/2 flex flex-col bg-white">
        ...
        <div className="p-6 md:p-8 overflow-y-auto flex-1 text-slate-700">{children}</div>
      </div>
    </div>
  );
};
...
const translateLatexToUnicode = (latex: string): string => {
  ...
  s = s.replace(/\\begin\{cases\}([\s\S]+?)\\end\{cases\}/g, (_match, body) => {
    ...
    return (
      ...
      translatedLines.map((line: string) => `<span style="white-space: nowrap;">${line}</span>`).join('') +
      `</span></span>`
    );
  });
...
export const MathRenderer: React.FC<MathRendererProps> = ({ text, useUnicodeFallback = false, className = '' }) => {
...
export const MiuMascot: React.FC<MiuMascotProps> = ({
    ...
    switch (mascotState) {
      case 'success':
        return {
          character: '😸✨',
          label: 'Happy Miu',
          moodStyle: 'bg-emerald-50 border-emerald-200 text-emerald-800',
        };
      case 'failed':
  ...
  return (
    <div
      className={`flex flex-col sm:flex-row items-center gap-4 p-4 border rounded-2xl shadow-sm transition-all duration-300 backdrop-blur-md bg-white/70 ${expression.moodStyle} ${className}`}
    >
      {/* Visual Mascot */}
        ...
        >
          <div style={{ maxWidth: 480, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.6, marginBottom: 16 }}>
              Ứng dụng gặp sự cố và không thể hiển thị màn hình này. Dữ liệu học tập của bạn vẫn an toàn. Hãy thử tải
              lại trang; nếu lỗi lặp lại, vui lòng liên hệ quản trị viên.
            </p>
            <pre
              style={{
                fontSize: 11,
