import { buildGraphBackendEvaluationReport, createSeedKnowledgeGraph, type GraphBackendEvaluationReport } from "./index";
...
console.log(`Status: ${report.status}; recommendation: ${report.recommendation}; criteriaMet: ${report.graphDbCriteriaMet}`);
...
function renderMarkdown(report: GraphBackendEvaluationReport): string {
  return [
    ...
    "| --- | --- | --- | --- |",
    ...report.triggers.map((trigger) => `| ${trigger.id} | ${trigger.met ? "yes" : "no"} | ${trigger.evidence} | ${trigger.recommendation} |`),
    "",
    "## Validation",
    "",
