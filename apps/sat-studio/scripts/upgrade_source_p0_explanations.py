...
VERSION = "source-p0-explanation-upgrade-2026-05-20"
...
def upgrade(question: dict[str, Any], source_file: str) -> None:
    ...
    if not isinstance(notes, dict):
        notes = {}
    notes["sourceP0ExplanationUpgradeVersion"] = VERSION
    notes["sourceP0ExplanationUpgradeSourceFile"] = source_file
    question["expertRepairNotes"] = notes


def process_source(source_file: str, apply: bool) -> dict[str, Any]:
    data_path = DATA_DIR / source_file
    report_path = DATA_DIR / f"{Path(source_file).stem}-p0-explanation-upgrade-report.json"
    ...
    return {k: v for k, v in report.items() if k not in {"attempted", "upgraded", "blocked", "remainingDepthFlags"}}
