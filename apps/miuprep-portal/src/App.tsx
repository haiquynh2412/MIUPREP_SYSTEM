...
export default function App() {
  ...
  const [mathLessons, setMathLessons] = useState<MathLesson[]>(INITIAL_MATH_LESSONS);
  const [mathCasioTips, setMathCasioTips] = useState<CasioTip[]>(INITIAL_CASIO_TIPS);
  ...
  const [importedExams, setImportedExams] = useState<ImportedExam[]>(() =>
    mergeImportedExamState(INITIAL_IMPORTED_EXAMS, loadPersistedImportedExams(localStorage)),
  );
  ...
  const [englishItemBankPracticeState, setEnglishItemBankPracticeState] = useState<EnglishItemBankPracticeState | null>(
    null,
  );
  ...
  const handleAnswerSatQuestion = (choice: string) => {
    ...
    if (currentUser?.role === 'student') {
      void saveStudentLearningEvent(
        buildSatPracticeLearningEvent(currentUser, currentQuestion, activePracticeState, choice, isCorrect),
      );
    }
  ...
  const startEnglishItemBankPractice = async (
    ...
    setShowErrorNotebook(false);
    showNotif(
      t('notif_start_itembank', { program: programId.toUpperCase(), count: nextState.questions.length }),
      'success',
    );
    return true;
  };
  ...
  const handleAnswerEnglishItemBankPractice = async (choice: EnglishItemBankPracticeChoiceKey) => {
    ...
    const { currentQuestion, isCorrect, nextState } = answerEnglishItemBankPracticeQuestion(
      englishItemBankPracticeState,
      choice,
    );
  ...
  const handleNextEnglishItemBankPractice = async () => {
    ...
    const { completed, finalScore, totalQuestions, nextState } =
      advanceEnglishItemBankPractice(englishItemBankPracticeState);
    if (completed) {
  ...
  const handleAdjustCoins = (username: string, amount: number) => {
    ...
    logSystemEvent(
      'WARN',
      `Admin @${currentUser?.username} đã điều chỉnh Salmon Coins cho @${username}: ${amount > 0 ? '+' : ''}${amount} xu. Số dư mới: ${newCoins}`,
    );
  ...
  const handleUpdateUserTracks = async (username: string, tracks: ('math' | 'sat' | 'ielts' | 'cae' | 'cpe')[]) => {
    try {
      ...
      logSystemEvent(
        'WARN',
        `Admin @${currentUser?.username} đã thay đổi phân quyền khoá học cho @${username}: [${tracks.join(', ')}]`,
      );
  ...
  const handleTerminalSubmit = async (e: React.FormEvent) => {
    ...
    if (primary === '/help') {
      await logSystemEvent(
        'INFO',
        'Lệnh hỗ trợ Admin: /seed (Tái tạo dữ liệu), /clear-logs (Xóa logs), /approve-all (Duyệt tất cả tài khoản), /coins @username [lượng] (Tặng xu)',
      );
  ...
  const handleAddEnglishExam = (trackId: EnglishExamTrack) => {
    ...
    logSystemEvent(
      'INFO',
      `Admin @${currentUser?.username} đã tạo đề thi ${trackId.toUpperCase()} mới: "${newExam.title}"`,
    );
  ...
  const handleRetryErrorQuestion = (qId: string, choice: string, correctAns: string) => {
    ...
    if (retryResult.isCorrect) {
      ...
      logSystemEvent(
        'INFO',
        `Học sinh @${currentUser?.username} đã giải đúng câu hỏi ôn tập Bẫy Chuột [${qId}], được thưởng +10 Xu`,
      );
  ...
  const handleRetryErrorQuestionV2 = (qId: string, choice: string, correctAns: string) => {
    ...
    if (retryResult.isCorrect) {
      ...
      logSystemEvent(
        'INFO',
        `Student @${currentUser?.username} resolved Error Notebook V2 question [${qId}], reward +10 coins`,
      );
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newUser: LocalUser = {
        ...
        examDate:
          regRole === 'student' ? new Date(Date.now() + 180 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] : '',
        createdAt: new Date().toISOString(),
      ...
      await logSystemEvent('INFO', `Tài khoản mới đăng ký: @${cleanUsername} [${regRole.toUpperCase()}]`, {
        username: cleanUsername,
        role: regRole,
      });
  ...
  const handleSaveDiary = (e: React.FormEvent) => {
    ...
    logSystemEvent('INFO', `Học sinh @${currentUser.username} viết nhật ký học tập, được tặng +15 Xu Cá Hồi`, {
      journal: studyDiary,
    });
  ...
  const handleDailyPlanCompleted = async () => {
    ...
    setStudentLearningEvents(await db.listLearningEvents(undefined, 200));
    await logSystemEvent('INFO', `Hoc sinh @${currentUser.username} hoan thanh today target`, {
      dateKey,
      eventId: event.id,
    });
  ...
  const handleEnglishLessonTemplateAction = async (
    template: LessonTemplateActionTemplate,
  ...
  const handleUpdateStudentTarget = async () => {
    ...
    try {
      ...
      if (studentObj) {
        ...
        await logSystemEvent(
          'INFO',
          `Phụ huynh @${currentUser?.username} cập nhật mục tiêu của con @${selectedStudent} thành ${weeklyTargetValue} buổi/tuần`,
        );
  ...
  const handleRewardCoins = async () => {
    ...
    try {
      ...
      await logSystemEvent(
        'INFO',
        `Phụ huynh @${currentUser?.username} khen thưởng con @${selectedStudent} thêm +${rewardAmount} Xu Cá Hồi`,
        { amount: rewardAmount, student: selectedStudent },
      );
  ...
  const handleUpdateStatus = async (userUsername: string, nextStatus: 'approved' | 'rejected') => {
    try {
      ...
      if (targetUser) {
        ...
        await logSystemEvent(
          'INFO',
          `Admin @${currentUser?.username} thay đổi trạng thái của @${userUsername} thành [${nextStatus.toUpperCase()}]`,
        );
  ...
  const TRACKS: TrackInfo[] = [
    {
      ...
      icon: '🧮',
      colorClass:
        'from-emerald-500/20 via-emerald-600/10 to-teal-900/40 hover:border-emerald-500/60 shadow-emerald-950/20',
    ...
    },
    {
      ...
      icon: '🎙️',
      colorClass:
        'from-indigo-500/20 via-indigo-600/10 to-indigo-950/40 hover:border-indigo-500/60 shadow-indigo-950/20',
    ...
    },
    {
      ...
      icon: '👑',
      colorClass:
        'from-purple-500/20 via-purple-600/10 to-purple-950/40 hover:border-purple-500/60 shadow-purple-950/20',
    ...
    },
    {
      ...
      icon: '✨',
      colorClass:
        'from-violet-500/20 via-violet-600/10 to-violet-950/40 hover:border-violet-500/60 shadow-violet-950/20',
  ...
  const effectiveAdminWorkspaceTab: AdminWorkspaceTabId =
    isAdminContentOnly && adminWorkspaceTab !== 'content' && adminWorkspaceTab !== 'analytics'
      ? 'content'
      : adminWorkspaceTab;
    ...
    if (!currentUser) {
      if (authTab === 'login') {
        ...
        const mascotMap = {
          student: {
            theme: 'math' as const,
            bubble: 'Trở thành Học sinh xuất sắc của MiuPrep để nhận Xu Cá Hồi meow meow! 🎒🐟',
          },
          parent: {
            theme: 'parent' as const,
            bubble: 'Chào Phụ huynh meow! Tạo tài khoản để cùng Miu đồng hành trên chặng đường học tập của con! 🏠👓',
          },
          admin: {
            theme: 'admin' as const,
            bubble: 'Chào mừng Quản trị viên meow! Điền mã bí mật để kích hoạt quyền Tổng quản hệ thống! 👑🛠️',
          },
        };
        return {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans selection:bg-indigo-500 selection:text-white pb-16">
      ...
      {notif && (
        <div
          className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-4 py-3 rounded-2xl border backdrop-blur-xl shadow-xl transition-all duration-300 animate-slide-in ${
            notif.type === 'success'
          ...
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            {currentUser ? (
              <div className="flex items-center gap-2 sm:gap-3 bg-slate-950 border border-slate-850 px-2 sm:px-4 py-1.5 rounded-full shadow-inner max-w-[calc(100vw-150px)] sm:max-w-none min-w-0">
                ...
                <span
                  className={`hidden sm:inline text-xs font-black uppercase text-slate-400 ${isAdminContentAccount(currentUser) ? 'sm:hidden' : ''}`}
                >
                ...
                {isAdminContentAccount(currentUser) && (
                  <span className="hidden sm:inline text-xs font-black uppercase text-indigo-300">
                    {t('header_admin_content')}
                  </span>
                )}
                <span className="text-xs font-semibold text-slate-200 truncate min-w-0 max-w-[88px] sm:max-w-none">
            ...
            {authTab === 'login' ? (
              <form onSubmit={handleLogin} className="flex flex-col gap-4">
                <div className="flex flex-col text-left">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-500 mb-1.5">
                ...
                <p className="text-[10px] text-slate-500 text-center mt-2">{t('auth_first_time_hint')}</p>
              </form>
            ) : (
              // REGISTER FORM
              <form onSubmit={handleRegister} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col text-left mb-2">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-500 mb-2">
                ...
                <div className="flex flex-col text-left">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-500 mb-1.5">
                    {t('auth_username_label_required')}
                  </label>
                  <input
                    type="text"
                    value={username}
                ...
                <div className="flex flex-col text-left">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-500 mb-1.5">
                    {t('auth_password_label_required')}
                  </label>
                  <input
                    type="password"
                    value={password}
                ...
                {regRole === 'parent' && (
                  <div className="flex flex-col text-left border-l-2 border-orange-500 pl-3 bg-orange-500/5 py-2 rounded-r-xl">
                    <label className="text-[10px] font-black uppercase tracking-wider text-orange-400 mb-1.5">
                      {t('auth_link_student_label')}
                    </label>
                    <input
                      type="text"
                      value={studentToLink}
                ...
                {regRole === 'admin' && (
                  <div className="flex flex-col text-left border-l-2 border-purple-500 pl-3 bg-purple-500/5 py-2 rounded-r-xl">
                    <span className="text-[10px] text-purple-300 leading-relaxed">{t('auth_admin_hint')}</span>
                  </div>
                )}
              ...
              {isAdminContentOnly && (
                <div className="mb-3 rounded-2xl border border-indigo-900/60 bg-indigo-950/25 p-4 text-left">
                  ...
                  <p className="mt-1 text-xs font-semibold text-slate-300">
                    Tai khoan nay chi quan ly noi dung de thi, review loi cau hoi, mo tung de, chinh sua truc tiep va
                    export change set.
                  </p>
                </div>
              )}
              <div
                className={`bg-slate-900/70 border border-slate-800 rounded-2xl p-2 grid grid-cols-2 ${isAdminContentOnly ? 'md:grid-cols-2' : 'md:grid-cols-5'} gap-2`}
              >
                {adminWorkspaceTabs.map((tab) => (
                  ...
                  >
                    ...
                    <span
                      className={`mt-1 block text-[10px] font-bold ${effectiveAdminWorkspaceTab === tab.id ? 'text-slate-900' : 'text-slate-600'}`}
                    >
                      {tab.detail}
                    </span>
                  </button>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-800">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1 font-light tracking-wide font-sans">
                      Cấu hình tham số mô hình học tập, quản lý ngân hàng câu hỏi thích ứng và import đề thi JSON nội
                      bộ.
                    </p>
                  </div>
