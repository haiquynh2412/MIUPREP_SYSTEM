...
import type { EnglishItemBankPracticeChoiceKey, EnglishItemBankPracticeState } from '../lib/englishItemBankPractice';
    ...
    <div className="space-y-12">
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-2 grid grid-cols-2 md:grid-cols-5 gap-2 shadow-xl">
          {studentWorkspaceTabs.map((tab) => (
            ...
            >
              ...
              <span
                className={`mt-1 block text-[10px] font-bold ${studentWorkspaceTab === tab.id ? 'text-slate-900' : 'text-slate-600'}`}
              >
                {tab.detail}
              </span>
            </button>
        ...
        <>
          ...
          <div>
            <h2 className="text-2xl font-black mb-6 text-center sm:text-left text-slate-200 uppercase tracking-widest flex items-center gap-2">
              ...
              <span className="text-xs bg-slate-900 border border-slate-800 text-slate-400 font-bold px-2 py-0.5 rounded-full">
                {tracks.length} TRACKS ACTIVE
              </span>
            </h2>
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 flex items-center justify-between shadow hover:border-indigo-500/40 transition-colors bg-gradient-to-tr from-slate-900 to-indigo-950/20">
              <div className="flex flex-col gap-1 text-left">
                ...
                <span className="text-[10px] text-slate-500">
                  Mục tiêu ôn tuần: {currentUser.studyPlan?.weeklyTarget || 4} buổi
                </span>
              </div>
              <span className="text-4xl filter drop-shadow">🌌</span>
            </div>
      ...
      {studentWorkspaceTab === 'tutor' && (
        <DeferredPanel label="Loading AI tutor">
          <AITutorPreviewPanel currentUser={currentUser} errorQuestions={errorQuestions} />
        </DeferredPanel>
      )}
          ...
          <section className="bg-slate-900/60 border border-rose-500/20 rounded-3xl p-6 max-w-4xl mx-auto shadow-xl text-left">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">{getErrorNotebookSummary(activeErrorQuestionCount)}</p>
              </div>
              <button
      ...
      {studentWorkspaceTab === 'practice' &&
        ...
        !templatePracticeState &&
        showErrorNotebook &&
        Boolean(localStorage.getItem('miuprep_show_legacy_error_notebook')) && (
          <section className="bg-slate-900/60 border-2 border-rose-500/30 rounded-3xl p-6 max-w-4xl mx-auto shadow-2xl relative overflow-hidden bg-gradient-to-r from-slate-900 to-rose-950/20 text-left transition-all duration-300">
            ...
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-black text-rose-400 uppercase tracking-widest flex items-center gap-2">
                ...
                <span className="text-[10px] bg-rose-950/60 text-rose-300 border border-rose-900 px-2 py-0.5 rounded-full font-bold font-mono">
                  ACTIVE REVIEW
                </span>
              </h3>
              <button
            ...
            <div className="space-y-4">
              {activeErrorQuestionCount === 0 ? (
                ...
                activeErrorQuestions.map((q) => (
                  <div
                    key={q.id}
                    className="p-4 bg-slate-955 rounded-2xl border border-slate-850 text-xs flex flex-col gap-3 relative"
                  >
                    <span className="absolute top-3 right-3 text-[9px] bg-rose-950/70 text-rose-400 border border-rose-900 px-2 py-0.5 rounded font-mono font-bold">
                      STAGE {q.stage}
                    </span>
                    ...
                    <details className="text-[10px] text-slate-500 font-light cursor-pointer select-none">
                      <summary className="font-bold text-slate-450 hover:text-slate-300">
                        {t('sdw_view_theory')}
                      </summary>
                      <p className="mt-1.5 bg-slate-900 p-2.5 rounded-lg border border-slate-850 leading-relaxed text-slate-400">
                        {q.answerExpl}
                      </p>
                    </details>
                  </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 pb-2 border-b border-slate-800">
            <div>
              <h3 className="text-sm font-black text-orange-400 uppercase tracking-widest flex items-center gap-2">
                ...
                <span className="text-[9px] bg-orange-950/60 text-orange-400 border border-orange-900 px-2 py-0.5 rounded-full font-black font-mono">
                  SALMON STORE
                </span>
              </h3>
              <p className="text-[10px] text-slate-500 mt-1 font-light">{t('sdw_shop_desc')}</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {MASCOT_STORE_ITEMS.map((item) => {
              ...
              return (
                <div
                  key={item.key}
                  className="p-4 bg-slate-955 rounded-2xl border border-slate-850 flex flex-col justify-between gap-3 transition-all hover:border-orange-500/30"
                >
                  <div className="flex items-start justify-between">
        ...
        <section className="bg-slate-900/30 border border-slate-850 rounded-3xl p-6 max-w-4xl mx-auto shadow-inner bg-gradient-to-br from-slate-900/50 to-indigo-950/15 text-left">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black text-indigo-400 uppercase tracking-widest flex items-center gap-2 font-mono">
              ...
              <span className="text-[9px] bg-indigo-950/60 text-indigo-400 border border-indigo-900 px-2 py-0.5 rounded-full font-bold">
                DESMOS LIVE
              </span>
            </h3>
            <button
        ...
        <section className="bg-slate-900/30 border border-slate-850 rounded-3xl p-6 max-w-4xl mx-auto shadow-inner bg-gradient-to-br from-slate-900/50 to-orange-950/15">
          ...
          <div className="max-h-48 overflow-y-auto flex flex-col gap-3 pr-2 scrollbar-thin text-left">
            {diaryList.map((entry, idx) => (
              <div
                key={idx}
                className="bg-slate-950/40 border border-slate-850 p-3 rounded-xl flex items-center justify-between gap-4"
              >
