    ...
    <div className="card" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '20px' }}>
      ...
      >
        ...
        <button
          ...
          style={{
            padding: '4px 12px',
            background: bookmarkedList.includes(currentQuestion.id) ? '#fffbeb' : '#f0faf5',
            border: '1px solid #10b981',
            fontSize: '0.85rem',
          }}
        ...
        >
          ...
          {hintLevel[currentQuestion.id] === 2 ? (
            <>
              ...
              <div
                style={{ background: '#fef2f2', padding: '16px', borderRadius: '16px', border: '1px solid #fecdd3' }}
              >
                <h5
                  style={{
                    color: '#ef4444',
      ...
      >
        ...
        <h3 style={{ color: '#064e3b', fontWeight: 700, fontSize: '1.15rem' }}>
          {mode === 'diagnostic_adaptive'
            ? `Adaptive Diagnostic: ${activeQuestions.length} câu`
            : `Chuyên đề: ${chuyenDeList[selectedChuyenDe].name}`}
        </h3>
      </div>
