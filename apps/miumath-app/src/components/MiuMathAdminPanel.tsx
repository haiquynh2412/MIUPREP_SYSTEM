    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {adminTab === 'members' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', width: '100%' }}>
          {(() => {
            ...
            const topHardWorking = [...studentMetrics].sort((a, b) => b.solvedCount - a.solvedCount).slice(0, 5);

            // 3. Đại Gia Xu Cá hồi
            const topFishCoins = [...studentMetrics].sort((a, b) => b.coins - a.coins).slice(0, 5);
            ...
            return (
              <>
                ...
                {pendingUsers.length > 0 ? (
                  ...
                  >
                    ...
                    <p style={{ color: '#d97706', fontSize: '0.82rem', marginBottom: '16px' }}>
                      Các tài khoản mới đăng ký dưới đây chưa thể đăng nhập. Vui lòng phê duyệt để cấp quyền truy cập hệ
                      thống meow!
                    </p>
                    <div style={{ overflowX: 'auto' }}>
                      <table
                        style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem', textAlign: 'left' }}
                      >
                        <thead>
                            ...
                            <tr key={u.username} style={{ borderBottom: '1px solid #fef3c7' }}>
                              <td style={{ padding: '10px' }}>
                                <div style={{ display: 'flex', flexDirection: 'column' }}>
                                  <strong style={{ color: '#78350f', fontSize: '0.92rem' }}>
                                    {u.fullName || 'Chưa đặt tên'}
                                  </strong>
                                  <span style={{ fontSize: '0.78rem', color: '#b45309' }}>@{u.username}</span>
                                </div>
                              </td>
                              <td style={{ padding: '10px', color: '#b45309', fontWeight: 500 }}>
                  ...
                  >
                    <span style={{ color: '#047857', fontSize: '0.85rem', fontWeight: 600 }}>
                      🐾 Không có thành viên nào đang chờ phê duyệt meow!
                    </span>
                  </div>
                )}
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {topScores.length === 0 ? (
                          <span style={{ fontSize: '0.82rem', color: '#6b7280', fontStyle: 'italic' }}>
                            Chưa có học sinh nào có điểm thi...
                          </span>
                        ) : (
                          topScores.map((s, idx) => (
                            <div
                ...
                <div
                  className="card"
                  style={{ background: '#ffffff', padding: '20px', border: '1px solid rgba(16, 185, 129, 0.1)' }}
                >
                  <h4 style={{ color: '#064e3b', fontWeight: 700, marginBottom: '14px' }}>
                              ...
                              <td style={{ padding: '10px' }}>
                                {u.role === 'admin' ? (
                                  ...
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', width: '160px' }}>
                                    ...
                                    >
                                      <span>
                                        Đã hiểu: {solvedList.length}/{questions.length} câu
                                      </span>
                                    ...
                                    >
                                      <div
                                        style={{ width: `${solvedPercent}%`, height: '100%', background: '#10b981' }}
                                      ></div>
                                    </div>
                                ...
                                <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      triggerMascotReaction(
                                        'success',
                                        `Meow! Bạn đang chuẩn bị sửa thông tin của @${u.username} đó! 🐾📝`,
                                      );
                                    }}
                                  >
                                    Sửa ✏️
      ...
      {adminTab === 'add_question' && (
        <div className="card" style={{ background: '#ffffff', padding: '20px' }}>
          ...
          >
            💡 **Mẹo Quản Trị**: Khi bạn thêm câu hỏi, dữ liệu sẽ được lưu tạm thời vào trình duyệt để bạn kiểm thử ngay
            lập tức. Để lưu trữ vĩnh viễn cho toàn bộ hệ thống web, hãy nhấn nút **Tải về file JSON Database mới**, sau
            đó chép đè tệp tải về vào đường dẫn thư mục dự án: `public/data/questions_db.json` và tiến hành deploy lên
            web!
          </p>
          ...
          <form onSubmit={handleAddQuestion} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '14px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#064e3b' }}>
                  Mã câu hỏi (ID - ví dụ: DE20_C23) *
                </label>
                <input
                  type="text"
                  value={newQId}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '14px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#064e3b' }}>
                  Chuyên đề con (Mã tiếng Anh - ví dụ: he-phuong-trinh)
                </label>
                <input
                  type="text"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#064e3b' }}>
                  Chuyên đề con (Tên tiếng Việt - hiển thị bộ lọc)
                </label>
                <input
                  type="text"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#064e3b' }}>
                Nội dung câu hỏi (hỗ trợ LaTeX kẹp trong $ hoặc $$) *
              </label>
              <textarea
                value={newQText}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '14px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#064e3b' }}>
                  Gợi ý cấp 1 (mở khóa bằng 5 xu)
                </label>
                <input
                  type="text"
                  value={newQHint1}
                  onChange={(e) => setNewQHint1(e.target.value)}
                  placeholder="Nhắc nhớ lý thuyết..."
                  style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #10b981' }}
                />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#064e3b' }}>
                  Gợi ý cấp 2 (mở khóa bằng 10 xu)
                </label>
                <input
                  type="text"
                  value={newQHint2}
                  onChange={(e) => setNewQHint2(e.target.value)}
                  placeholder="Định hướng hướng đi chính..."
                  style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #10b981' }}
                />
              </div>
            </div>
            ...
            >
              <span style={{ fontSize: '0.9rem', fontWeight: 700, color: '#064e3b' }}>
                📘 Thiết lập lời giải chuẩn MiuMath (Bắt buộc 2 phần mục sư phạm)
              </span>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: '#15803d' }}>
                  Phần 1: Nhìn vào đề có gì đặc biệt? *
                </label>
                <textarea
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: '#15803d' }}>
                  Phần 2: Tại sao lại nghĩ ra cách làm như vậy? *
                </label>
                <textarea
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 600, color: '#15803d' }}>
                  Các bước giải chi tiết (các bước cụ thể) *
                </label>
                <textarea
                  value={newQSteps}
