function ExamQuestionNavigator({ activeQuestions, currentQuestionIndex, examAnswers, examFinished, setCurrentQuestionIndex }) {
  return (
    <div className="card" style={{ padding: "16px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "8px" }}>
        {activeQuestions.map((question, index) => {
          ...
          if (examFinished) {
            const isCorrect = (examAnswers[question.id] || "").trim().toLowerCase() === question.correct_answer.trim().toLowerCase();
...
function ExamActiveQuestion({
  ...
  return (
    <div className="card">
      ...
      {examFinished && <ExamExplanation currentQuestion={currentQuestion} examAnswers={examAnswers} renderMath={renderMath} />}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "20px", borderTop: "1px solid var(--color-border)", paddingTop: "16px" }}>
        <button className="btn btn-secondary" disabled={currentQuestionIndex === 0} onClick={() => setCurrentQuestionIndex((previousIndex) => previousIndex - 1)}>
          👈 Câu trước
        </button>
        <button className="btn btn-secondary" disabled={currentQuestionIndex === activeQuestions.length - 1} onClick={() => setCurrentQuestionIndex((previousIndex) => previousIndex + 1)}>
          Câu tiếp theo 👉
        </button>
      </div>
    </div>
...
export default function MathExamRoom({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", flexWrap: "wrap", background: "#ffffff", padding: "16px", borderRadius: "16px", border: "2px solid var(--color-border)" }}>
        <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(220px, 280px)", gap: "20px", alignItems: "start" }}>
        <ExamActiveQuestion
