    ...
    <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginTop: "12px" }}>
      {!isAnswerCorrect[questionId] && hintLevel[questionId] !== 2 && (
        <button className="btn btn-primary" onClick={() => handleSubmitAnswer(questionId)} disabled={!hasAnswer}>
          {hintLevel[questionId] === 1 ? (isFillInBlank ? "💡 Điền lại đáp án lần 2" : "💡 Nộp bài lại lần 2") : "⚡ Giải lại bẫy này"}
        </button>
      )}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div className="card" style={{ background: "#fef2f2", borderColor: "#fecdd3" }}>
        <p style={{ fontSize: "0.92rem", color: "#991b1b" }}>
          🐱 <strong>Mèo Miu khuyên học tập</strong>: Đây là danh sách các cạm bẫy {userName} đã vướng phải trong các bài tập!
          Hãy ôn luyện giải lại các câu này theo chu kỳ <strong>1 ngày, 3 ngày, 7 ngày</strong> để khắc cốt ghi tâm meow meow!
        </p>
      </div>
      ...
      {currentQuestion ? (
        <div className="card">
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "16px", borderTop: "1px solid var(--color-border)", paddingTop: "16px" }}>
            ...
            <button className="btn btn-secondary" disabled={currentQuestionIndex === activeQuestions.length - 1} onClick={goToNextTrap}>
              Câu tiếp theo 👉
            </button>
          </div>
        </div>
