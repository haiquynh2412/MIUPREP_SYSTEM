...
const translateLatexToUnicode = (latex) => {
  ...
  s = s.replace(
    /\\sqrt\{([\s\S]+?)\}/g,
    '<span style="display: inline-flex; align-items: baseline; white-space: nowrap;"><span style="font-family: inherit; font-size: 1.1em; transform: translateY(1px); line-height: 1; margin-right: 0.5px;">√</span><span style="border-top: 1.2px solid currentColor; padding-top: 1px; margin-left: 0.5px;">$1</span></span>',
  );
  s = s.replace(/\\sqrt/g, '√');
...
function App() {
  ...
  const handleLogin = async (e) => {
    ...
    const foundUser = users.find((u) => u.username.toLowerCase() === authUsername.trim().toLowerCase());
    if (!foundUser) {
    ...
    if (verdict.needsRehash) {
      ...
      const upgradedUsers = users.map((u) =>
        u.username === foundUser.username ? { ...u, password: upgradedHash } : u,
      );
    ...
    if (foundUser.approved === false) {
      setAuthError(
        'Tài khoản của bạn đang chờ Quản trị viên phê duyệt meow! Vui lòng liên hệ Giáo viên hoặc thử lại sau nhé! ⏳🐾',
      );
      return;
    }
    ...
    setMascotBubble(
      `Meow! Chào mừng ${foundUser.username} (${foundUser.role === 'admin' ? 'Quản trị viên' : 'Học sinh'}) đã đăng nhập thành công! 🎉`,
    );
  };
  ...
  const handleRegister = async (e) => {
    ...
    const exist = users.some((u) => u.username.toLowerCase() === authUsername.trim().toLowerCase());
    if (exist) {
  ...
  const handleApproveUser = (username) => {
    ...
    triggerMascotReaction(
      'success',
      `Meow meow! Quản trị viên đã phê duyệt kích hoạt tài khoản cho @${username} thành công! 🎉🐾`,
    );
  };
  ...
  const handleSubmitAnswer = (qId, optionKey = null) => {
    ...
    if (hintLevel[qId] !== 2) {
      const attemptMode =
        mode === 'diagnostic_adaptive' ? 'diagnostic' : mode === 'so_tay_bay_chuot' ? 'review' : 'practice';
    ...
    if (isCorrect) {
      ...
      if (currentLevel === 0) {
        ...
        if (recorded?.attempt) {
          const notebook = recordMiuMathErrorNotebookMistake(
            currentLearnerId,
            q,
            currentAnswer,
            recorded.attempt,
  ...
  const handleBuyHint = (qId) => {
    if (fishCoins >= 15) {
      ...
      triggerMascotReaction(
        'hint',
        `Meow meow! Miu đã dùng phép thuật mở khóa Gợi ý Tư duy cho ${userName} rồi nhé! Đọc kỹ gợi ý để tự giải ra bài toán meow! 🐾✨`,
      );
    } else {
      triggerMascotReaction(
        'failed',
        `Meow meow... Số dư xu cá hồi không đủ rồi ${userName} ơi! Hãy làm đúng các câu hỏi hoặc hoàn thành đề thi thử để kiếm thêm xu nhé! 🐟😿`,
      );
    }
  };
  ...
  const handleBuySolution = (qId) => {
    if (fishCoins >= 30) {
      ...
      triggerMascotReaction(
        'success',
        `Meow meow! Toàn bộ lời giải chi tiết, cạm bẫy và bí kíp Casio đã được mở khóa rồi meo! Chúc ${userName} học thật tốt nhé! 🌟🐾`,
      );
    } else {
      triggerMascotReaction(
        'failed',
  ...
  const handleStartExam = (examId) => {
    ...
    triggerMascotReaction(
      'idle',
      'Bắt đầu làm bài thi thử nào meow! Hãy giữ tập trung cao độ và căn thời gian chuẩn nhé!',
    );
  };
  ...
  const handleFinishExam = () => {
    ...
    examQuestions.forEach((q) => {
      ...
      const recorded = recordMiuMathAttempt(
        nextLearningState,
        q,
        userAns || '__blank__',
        'mock_test',
    ...
    if (currentUser) {
      const updatedUsers = users.map((u) =>
        u.username === currentUser.username ? { ...u, currentScore: scoreStr } : u,
      );
      setUsers(updatedUsers);
      localStorage.setItem('miu_math_users', JSON.stringify(updatedUsers));
      ...
      ? questions.filter((q) => {
          const matchCategory =
            currentChuyenDeId === 'casio-hacks'
              ? q.category === 'casio-hacks' ||
                Boolean(
                  typeof q.explanation === 'object' &&
                  q.explanation !== null &&
                  (q.explanation as Record<string, unknown>).casio,
                )
  ...
  const openRecommendedQuestion = () => {
    ...
    if (!recommended) {
      triggerMascotReaction(
        'idle',
        'Learning core chua tim thay cau goi y moi. Hay lam them vai cau diagnostic hoac chuyen de truoc nhe.',
      );
      return;
    }
    ...
    const categoryIndex = CHUYEN_DE_LIST.findIndex((cd) => {
      ...
      return (
        cd.id === 'casio-hacks' &&
        (recommended.category === 'casio-hacks' ||
          Boolean(
            typeof recommended.explanation === 'object' &&
            recommended.explanation !== null &&
            (recommended.explanation as Record<string, unknown>).casio,
          ))
      );
    });
    ...
    const targetQuestions = questions.filter((q) => {
      if (nextCategory.id === 'casio-hacks')
        return (
          q.category === 'casio-hacks' ||
          Boolean(
            typeof q.explanation === 'object' &&
  ...
  const startAdaptiveDiagnostic = () => {
    ...
    if (!selected.length) {
      triggerMascotReaction(
        'idle',
        'Chua co du cau hoi diagnostic moi. Hay kiem tra ngan hang cau hoi hoac lam them bai chuyen de.',
      );
      return;
    }
  ...
  return (
    <div className="App">
      ...
      <footer
        style={{
          background: '#ffffff',
          borderTop: '2px solid rgba(16, 185, 129, 0.08)',
          padding: '20px 16px',
          textAlign: 'center',
          marginTop: '40px',
        }}
