...
export default function SpeakingAiRoom({
  ...
  const [selectedSpeakingSampleId, setSelectedSpeakingSampleId] = useState(CPE_SPEAKING_SAMPLES[0].id);
  ...
  useEffect(() => {
    const currentSample = CPE_SPEAKING_SAMPLES.find(s => s.prompt === speakingTopic) || CPE_SPEAKING_SAMPLES[0];
    setTimeout(() => {
  ...
  }, [speakingTopic, setSpeakingTopic]);

  const handleSpeakingSampleChange = (id: string) => {
    setSelectedSpeakingSampleId(id);
    const found = CPE_SPEAKING_SAMPLES.find(s => s.id === id);
    if (found) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
      ...
      <div className="md:col-span-2 bg-white border border-slate-200 rounded-lg p-6 shadow-sm flex flex-col gap-5">
        ...
        {/* Speech topics */}
        {(() => {
          const activeSpeakingSample = CPE_SPEAKING_SAMPLES.find(s => s.id === selectedSpeakingSampleId) || CPE_SPEAKING_SAMPLES[0];
          return (
            <div className="flex flex-col gap-4 text-left">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-wide">
                  Chọn chủ đề nói thử thách Cambridge CPE 2:
                </label>
                <select
                  value={selectedSpeakingSampleId}
                  onChange={(e) => handleSpeakingSampleChange(e.target.value)}
                  className={`bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs focus:ring-1 cursor-pointer font-bold text-slate-700 focus:outline-none w-full ${
                    isCpeCae ? 'focus:ring-emerald-500 focus:border-emerald-500' : 'focus:ring-blue-500 focus:border-blue-500'
                  }`}
                >
                  {CPE_SPEAKING_SAMPLES.map(sample => (
              ...
              <div className="border border-emerald-100 bg-emerald-50/20 rounded-xl p-4 flex flex-col gap-3">
                <div className="flex justify-between items-center border-b pb-1.5">
                  <span className="text-[9px] bg-emerald-100 border border-emerald-200 text-emerald-800 font-bold px-2 py-0.5 rounded-full uppercase">
                    Cambridge CPE Part {activeSpeakingSample.part} Speaking
                  </span>
                  <span className="text-[10px] text-slate-400 font-mono">
                    C2 Proficiency
                  </span>
                </div>
                <p className="m-0 font-serif italic text-slate-750 text-xs leading-relaxed select-all">
                  "{activeSpeakingSample.prompt}"
                </p>
                <button
                  onClick={() => setShowSpeakingOutlineModal(true)}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-2 px-3.5 rounded-lg shadow-sm transition-all border-0 cursor-pointer outline-none flex items-center justify-center gap-1.5 uppercase tracking-wide"
                >
                  <span>💡</span> Dàn ý & Bài nói mẫu C2
                </button>
              </div>

              {/* Vocabulary Booster Box (Lexical Upgrades) */}
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs text-slate-750 flex flex-col gap-2.5">
                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-wider m-0 flex items-center gap-1">
                  🚀 Nâng cấp từ vựng C2 ăn điểm (Lexical upgrades):
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-1">
                  {activeSpeakingSample.lexicalUpgrades.map((upg, idx) => (
                    <div key={idx} className="bg-white border rounded p-2.5 flex flex-col gap-1 shadow-sm">
                      ...
                      <span className="font-bold font-mono text-emerald-700 text-[10px] leading-tight">
                        {upg.upgrade}
                      </span>
            ...
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl flex items-center justify-between text-white text-left">
              <div>
                <span className="text-[10px] text-slate-400 uppercase font-black tracking-wider">
                  {isCpeCae ? 'C2 Proficiency Grading Verification' : 'IELTS Grading Verification'}
                </span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 border-t border-slate-100 pt-3 text-left">
                        ...
                        {crit.nextAction && (
                          <div className="flex flex-col gap-1 bg-green-50/50 border border-green-100 p-3 rounded-lg text-left">
                            ...
                            <p className="text-[11px] text-green-950 leading-relaxed m-0">{crit.nextAction}</p>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {speakingFeedback.sentenceUpgrades && speakingFeedback.sentenceUpgrades.length > 0 && (
              <div className="border-t border-slate-100 pt-4 mt-2 text-left">
                <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wide mb-3 flex items-center gap-1.5 text-left">
                  <span>🚀</span> Chiến Lược Nâng Điểm (+0.5 Speaking Upgrades)
                </h4>
                <div className="flex flex-col gap-4 text-left">
      ...
      {/* Speaking Outline & Sample Modal */}
      {showSpeakingOutlineModal && (() => {
        const activeSpeakingSample = CPE_SPEAKING_SAMPLES.find(s => s.id === selectedSpeakingSampleId) || CPE_SPEAKING_SAMPLES[0];
        return (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in text-left">
            <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden flex flex-col max-h-[85vh]">
              ...
              <div className="flex bg-slate-50 border-b px-6 pt-2">
                ...
                >
                  🗣️ Bản nói mẫu Native Transcript
                </button>
                <button
              ...
              <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-4 text-xs text-slate-705 leading-relaxed">
                {speakingModalTab === 'outline' && (
                  <div className="flex flex-col gap-3">
                    <p className="text-slate-500 m-0">
                      Cấu trúc triển khai ý tưởng nói giúp tối ưu tiêu chí **Discourse Management** (mạch lạc, trôi chảy):
                    </p>
                    <div className="flex flex-col gap-3 mt-1">
                      {activeSpeakingSample.outline.map((step, idx) => (
                        <div key={idx} className="border-l-4 border-emerald-600 bg-slate-50 p-3.5 rounded-r-lg shadow-sm">
                          <h4 className="font-bold text-slate-800 m-0 text-xs">
                            Bước {idx + 1}
                          </h4>
                          <p className="m-0 text-slate-650 mt-1 leading-relaxed">
                            {step}
                          </p>
                        </div>
                      ))}
                ...
                {speakingModalTab === 'sample' && (
                  <div className="flex flex-col gap-3">
                    <div className="bg-slate-50 border p-3 rounded-lg flex items-center justify-between text-slate-600 font-semibold">
                      <span>Bản phát âm mẫu chuẩn giọng Anh-Mỹ C2 (Native Level):</span>
                    </div>
                    <div className="bg-slate-900 text-slate-100 rounded-xl p-5 font-serif text-[12.5px] leading-relaxed max-h-[35vh] overflow-y-auto shadow-inner select-all whitespace-pre-line border border-slate-800">
                      "{activeSpeakingSample.sampleTranscript}"
                    </div>
                    <div className="flex gap-2.5 mt-1 justify-end">
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(activeSpeakingSample.sampleTranscript);
                          alert('Đã sao chép bài nói mẫu vào bộ nhớ tạm!');
                        }}
                      ...
                      >
                        📋 Sao chép bài nói mẫu
                      </button>
                      <button
                        onClick={() => {
                          alert('Mẹo tập nói Shadowing:\n1. Nghe kỹ hoặc đọc to bài nói mẫu trên.\n2. Bật ghi âm của phòng luyện nói.\n3. Nói nhại theo tông giọng và các từ vựng đắt giá để AI chấm điểm đạt band tuyệt đối!');
                        }}
                ...
                {speakingModalTab === 'vocab' && (
                  <div className="flex flex-col gap-3">
                    <p className="text-slate-500 m-0">
                      Cách chuyển hóa từ vựng thông dụng (Basic) sang cấp độ học thuật chuẩn C2 (Proficiency):
                    </p>
                    <div className="flex flex-col gap-3 mt-1">
                      {activeSpeakingSample.lexicalUpgrades.map((upg, idx) => (
                        <div key={idx} className="bg-slate-50 border rounded-lg p-3.5 flex flex-col gap-2 shadow-sm">
                          <div className="flex items-center gap-2">
                            <span className="bg-red-50 text-red-650 border border-red-150 font-mono text-[9px] px-1 py-0.2 rounded font-bold line-through">
                              "{upg.basic}"
                            </span>
                            <span className="text-slate-400">➔</span>
                            <span className="bg-emerald-600 text-white font-mono text-[10px] px-2 py-0.2 rounded font-black">C2 Upgrade</span>
                            <strong className="font-bold text-emerald-800 text-xs font-mono">{upg.upgrade}</strong>
                          </div>
                          <div className="text-[11px] text-slate-650 font-medium">
                            Ý nghĩa học thuật: <span className="text-slate-800 font-semibold">{upg.meaning}</span>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <footer className="bg-slate-50 px-6 py-3.5 flex justify-end border-t border-slate-150">
                <button
                  onClick={() => setShowSpeakingOutlineModal(false)}
                  className="bg-slate-200 hover:bg-slate-350 text-slate-700 font-bold text-xs py-2 px-4 rounded-lg shadow-sm transition-all border-0 cursor-pointer outline-none uppercase"
                >
                  Đóng lại
                </button>
              </footer>
