          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {availableTests
              ...
              .map((test) => (
                <div
                  key={test.id}
                  className="border border-slate-200 rounded-lg p-5 bg-slate-50 flex flex-col justify-between shadow-sm text-left hover:border-emerald-300 transition-all duration-300 transform hover:-translate-y-0.5"
                >
                  <div>
                    <div className="flex justify-between items-start gap-2">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase bg-emerald-100 text-emerald-800`}
                      >
                        {test.type}
                      </span>
      ...
      <div className="flex flex-col gap-6 text-left">
        ...
        {isAdmin && (
          <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-sm flex flex-col gap-4">
            ...
            <p className="text-xs text-slate-500 m-0">
              Upload an independently compiled exam package in `.json` format to load it instantly. The engine will run
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-6 shadow-sm">
          ...
          <ul className="text-sm text-emerald-800 flex flex-col gap-2 list-disc list-inside p-0 m-0">
            ...
            <li>
              In **Reading**, you will see a two-panel split layout: the passage text on the left, and the input sheet
              on the right.
            </li>
            <li>In **Listening**, the audio is controlled via a locked media player that allows only a single run.</li>
            <li>
              The countdown timers utilize **drift-proof clock calculations** to remain accurate even if your device
              sleeps.
            </li>
          </ul>
        </div>
