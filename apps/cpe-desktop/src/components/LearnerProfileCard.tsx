...
export default function LearnerProfileCard({
  ...
  const [lastSynced, setLastSynced] = useState<string | null>(() => {
    return localStorage.getItem('cpe_app_last_cloud_sync');
  });
  ...
  const plannerAdvice = generateStudyPlannerAdvice({
    targetBand: learnerProfile?.targetBand || 7.0,
    ...
    steps.forEach((step, idx) => {
      setTimeout(() => {
        ...
        if (step.progress === 100) {
          ...
          localStorage.setItem('cpe_app_last_cloud_sync', timestamp);
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-4 mb-4">
          ...
          {!isEditingProfile ? (
            <button
              ...
              className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs py-1.5 px-3 rounded shadow-sm border transition-all cursor-pointer"
            >
              Chỉnh sửa hồ sơ
            </button>
          ) : (
            <div className="flex gap-2">
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-1.5 px-3 rounded shadow-sm transition-all cursor-pointer"
              >
                Lưu hồ sơ
              </button>
              <button
                onClick={() => setIsEditingProfile(false)}
                className="bg-white hover:bg-slate-50 text-slate-600 font-bold text-xs py-1.5 px-3 rounded shadow-sm border transition-all cursor-pointer"
              >
                Hủy
              </button>
            </div>
        ...
        {isEditingProfile ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 border p-4 rounded-lg mb-4 text-sm">
            <div className="flex flex-col gap-1.5">
              <label className="font-bold text-slate-700">Target CPE Score (180-230):</label>
              <input
                type="number"
                min="180"
                max="230"
                ...
                className="border border-slate-300 rounded px-2.5 py-1.5 text-slate-800 bg-white focus:outline-none focus:border-emerald-500 font-medium"
              />
            </div>
            <div className="flex flex-col gap-1.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
            <div className="bg-gradient-to-r from-emerald-50 to-emerald-50 border border-emerald-100 rounded-lg p-4 flex flex-col justify-center items-center text-center shadow-sm">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                Điểm CPE mục tiêu
              </span>
              <span className="text-3xl font-black text-emerald-600 mt-1">
                {learnerProfile?.targetBand ? learnerProfile.targetBand.toFixed(0) : '200'}
              </span>
            </div>

            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 rounded-lg p-4 flex flex-col justify-center items-center text-center shadow-sm">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                Ngày thi dự kiến
              </span>
      ...
      <div className="bg-gradient-to-r from-emerald-600 to-emerald-600 text-white rounded-lg p-6 shadow-md relative overflow-hidden flex flex-col gap-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm font-semibold leading-relaxed mt-2 text-emerald-50">
        ...
        {plannerAdvice.status !== 'new' && onNavigateTab && (
          <button
            ...
            className="self-start mt-1 bg-white hover:bg-slate-50 text-emerald-700 text-xs font-black py-2.5 px-5 rounded-lg shadow-md transition-all cursor-pointer uppercase tracking-wider"
          >
      ...
      <div className="bg-slate-900 border border-slate-800 text-slate-100 rounded-lg p-6 shadow-md flex flex-col gap-4 relative">
        <h3 className="text-base font-bold text-white flex items-center gap-2">
          <svg className="w-5 h-5 text-emerald-400 fill-current" viewBox="0 0 24 24">
            <path d="M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM14 13v4h-4v-4H7l5-5 5 5h-3z" />
          </svg>
          CPE Local Backup Simulator
        </h3>
        ...
        {isSyncing ? (
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mt-2">
            ...
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-2.5 px-4 rounded shadow transition-all cursor-pointer whitespace-nowrap self-stretch sm:self-auto text-center"
            >
