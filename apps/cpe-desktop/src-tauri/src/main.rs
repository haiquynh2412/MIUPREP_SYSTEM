...
fn register_local_user(
  ...
  let conn = state.conn.lock().map_err(|e| format!("Database lock poisoned: {}", e))?;
  conn.execute(
    "INSERT OR REPLACE INTO local_users (id, username, password_hash, target_band, exam_date, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
    params![user.id, user.username, user.password_hash, user.target_band, user.exam_date, user.created_at]
  ).map_err(|e| e.to_string())?;
  Ok(())
}
...
fn get_local_user(state: State<DbState>, username: String) -> Result<Option<serde_json::Value>, String> {
  let conn = state.conn.lock().map_err(|e| format!("Database lock poisoned: {}", e))?;
  let mut stmt = conn.prepare("SELECT id, username, password_hash, target_band, exam_date, created_at FROM local_users WHERE username = ?1 LIMIT 1").map_err(|e| e.to_string())?;
...
fn list_local_users(state: State<DbState>) -> Result<Vec<serde_json::Value>, String> {
  let conn = state.conn.lock().map_err(|e| format!("Database lock poisoned: {}", e))?;
  let mut stmt = conn.prepare("SELECT id, username, target_band, exam_date, created_at FROM local_users").map_err(|e| e.to_string())?;
  let mut rows = stmt.query(params![]).map_err(|e| e.to_string())?;
  let mut results = Vec::new();
