      ...
      <div key={q.id} className="border border-slate-200 bg-slate-50/50 p-6 rounded-2xl flex flex-col gap-4 text-xs text-slate-750 text-left shadow-sm">
        ...
        {q.type === 'gap_fill' && (
          <input
            ...
            className="border border-slate-300 rounded-xl px-3.5 py-2 focus:outline-none focus:border-indigo-500 bg-white text-slate-800 max-w-sm font-semibold shadow-inner"
          />
        )}
              ...
              return (
                <label
                  ...
                  className={`flex items-center gap-2.5 cursor-pointer border px-4 py-2.5 rounded-xl text-xs transition-all ${
                    isSelected
                      ? 'bg-indigo-50 border-indigo-300 font-bold text-indigo-950 shadow-sm'
                      : 'bg-white border-slate-200 hover:bg-slate-50'
                  } ${isChecked ? 'cursor-not-allowed' : ''}`}
                >
                  <input
        ...
        {!isChecked && (
          <div className="mt-1 text-left">
            <details className="cursor-pointer group">
              ...
              <div className="mt-2 p-3 bg-indigo-50 border border-indigo-100 rounded-xl text-[10px] leading-relaxed text-indigo-950 font-sans shadow-sm">
        ...
        <div className="flex items-center justify-between border-t pt-4 mt-3">
          {!isChecked ? (
            <button
              ...
              className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold text-xs py-2.5 px-5 rounded-xl shadow transition-all border-0 outline-none cursor-pointer disabled:cursor-not-allowed ml-auto"
            >
              Kiểm tra đáp án ✓
            </button>
          ) : (
            <>
              ...
              {currentPracticeQuestionIdx < questionsInGroup.length - 1 ? (
                <button
                  ...
                  className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs py-2.5 px-5 rounded-xl cursor-pointer border-0 transition-all outline-none shadow-sm ml-auto"
                >
                  Câu tiếp theo →
                </button>
                ...
                <div className="flex flex-col gap-4 animate-fade-in pr-1">
                  ...
                  <div className="flex flex-col gap-4">
                    {paragraphs.map((p: any) => (
                      <div 
                        key={p.label} 
                        className="bg-slate-50/50 hover:bg-indigo-50/10 border border-slate-200 hover:border-indigo-300 rounded-xl p-4 flex gap-4 text-left transition-all duration-300 shadow-sm"
                      >
                        <div className="w-7 h-7 bg-indigo-600 border border-indigo-500 text-white rounded-full flex items-center justify-center font-black font-mono text-xs shrink-0 shadow-sm">
                          {p.label}
                        </div>
