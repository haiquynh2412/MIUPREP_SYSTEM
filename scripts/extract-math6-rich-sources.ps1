param(
  ...
  [string]$OutPath = "reports\content-quality\math6-rich-raw-extract.json",
  [string]$HtmlWorkDir = "reports\content-quality\math6-rich-html",
  [string]$AssetPublicRoot = "apps\miuprep-portal\public\assets\math6\formulas",
  [switch]$IncludePdf,
  [switch]$DisableEmfFallback,
  [string]$FileListPath = "",
  [int]$Limit = 0
)
...
function Get-HtmlAttribute {
  ...
  for ($i = 1; $i -lt $match.Groups.Count; $i += 1) {
...
function Read-HtmlFile {
  ...
  $charsetMatch = [regex]::Match($probe, 'charset\s*=\s*"?([^"\s>]+)', 'IgnoreCase')
  $encodingName = if ($charsetMatch.Success) { $charsetMatch.Groups[1].Value } else { "utf-8" }
...
function Resolve-HtmlAssetPath {
  param(
    [string]$HtmlPath,
    [string]$Src
  )
  ...
  if ($clean.StartsWith("file:///", [System.StringComparison]::OrdinalIgnoreCase)) {
    try {
      return ([System.Uri]$Src).LocalPath
    } catch {
      return ""
    }
  }
  ...
  return Join-Path (Split-Path -Parent $HtmlPath) $clean
}

function Convert-HtmlToRichText {
  param(
    [string]$HtmlPath,
  ...
  $html = Read-HtmlFile -HtmlPath $HtmlPath
  ...
  $body = [regex]::Replace($body, '(?is)<img\b[^>]*>', {
    ...
    $tag = $match.Value
    $src = Get-HtmlAttribute -Tag $tag -Name "src"
    if (-not $src) {
      return " "
    }

    $width = Get-HtmlAttribute -Tag $tag -Name "width"
    $height = Get-HtmlAttribute -Tag $tag -Name "height"
    $sourceAssetPath = Resolve-HtmlAssetPath -HtmlPath $HtmlPath -Src $src
    $assetName = [System.IO.Path]::GetFileName($sourceAssetPath)
    if (-not $assetName) {
      $assetName = [System.IO.Path]::GetFileName(($src -replace '/', '\'))
    }
    if (-not $assetName) {
      $assetName = "formula-$($assets.Count + 1).gif"
    }

    $targetPath = Join-Path $AssetTargetDir $assetName
    $copied = $false
    if ($sourceAssetPath -and (Test-Path -LiteralPath $sourceAssetPath)) {
      try {
        Copy-Item -LiteralPath $sourceAssetPath -Destination $targetPath -Force
        $copied = $true
      } catch {
        $copied = $false
      }
    ...
    $assetRecord = [ordered]@{
      src = $publicPath
      width = if ($width) { $width } else { "" }
      height = if ($height) { $height } else { "" }
      fileName = $assetName
      copied = $copied
      source = $src
    }
  ...
  $body = [regex]::Replace($body, '(?is)</(?:p|div|h[1-6]|li|tr|table|section|article)\s*>', "`n")
  $body = [regex]::Replace($body, '(?is)<(?:p|div|h[1-6]|li|tr|table|section|article)\b[^>]*>', "`n")
  $body = [regex]::Replace($body, '(?is)</td\s*>', "`t")
  $body = [regex]::Replace($body, '(?is)<[^>]+>', ' ')
  $text = [System.Net.WebUtility]::HtmlDecode($body)
...
function Export-InlineShapeAsset {
  ...
  if (-not $copied -and -not $DisableEmfFallback) {
...
function Write-ExtractPayload {
  ...
  $payload = [ordered]@{
    schemaVersion = "math6_rich_raw_extract_v1"
    generatedAt = (Get-Date).ToUniversalTime().ToString("o")
    sourceRoot = $SourceRoot
    assetPublicRoot = "/assets/math6/formulas"
    sources = $Sources
  }
...
try {
  ...
  foreach ($file in $files) {
    ...
    $assetPublicBase = "/assets/math6/formulas/$sourceSlug"
