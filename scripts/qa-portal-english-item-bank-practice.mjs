...
for (const programId of programs) {
  ...
  if ((await openPracticeButtons.count()) < 2) {
    throw new Error(
      `Expected Math and English Open practice buttons for ${programId}, got ${await openPracticeButtons.count()}`,
    );
  }
  ...
  const proof = await page.evaluate(
    ({ qaUsername, programId }) => {
      ...
      const itemBankPracticeEvents = practiceEvents.filter(
        (event) => event.source === 'miuprep_portal_english_item_bank_practice',
      );
