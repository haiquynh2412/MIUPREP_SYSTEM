param(
  [string]$SourceRoot = "C:\Users\HAIQUYNH\OneDrive\SACH VIET\TOAN\toan 7",
  [string]$OutPath = "reports\content-quality\math7-rich-raw-extract.json",
  [string]$HtmlWorkDir = "reports\content-quality\math7-rich-html",
  [string]$AssetPublicRoot = "apps\miuprep-portal\public\assets\math7\formulas",
  [switch]$IncludePdf,
  [string]$FileListPath = "",
  [int]$Limit = 0
)
...
function Write-ExtractPayload {
  ...
  $payload = [ordered]@{
    schemaVersion = "math7_rich_raw_extract_v1"
    generatedAt = (Get-Date).ToUniversalTime().ToString("o")
    sourceRoot = $SourceRoot
    assetPublicRoot = "/assets/math7/formulas"
    sources = $Sources
  }
...
try {
  ...
  foreach ($file in $files) {
    ...
    $assetPublicBase = "/assets/math7/formulas/$sourceSlug"
