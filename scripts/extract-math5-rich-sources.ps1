param(
  [string]$SourceRoot = "C:\Users\HAIQUYNH\OneDrive\SACH VIET\TOAN\CHIA SẺ TÀI LIỆU TOÁN LỚP 3-4-5-6\TAI LIEU TOAN 5",
  [string]$OutPath = "reports\content-quality\math5-rich-raw-extract.json",
  [string]$HtmlWorkDir = "reports\content-quality\math5-rich-html",
  [string]$AssetPublicRoot = "apps\miuprep-portal\public\assets\math5\formulas",
  [switch]$IncludePdf,
  [string]$FileListPath = "",
  [int]$Limit = 0
)
...
function Write-ExtractPayload {
  ...
  $payload = [ordered]@{
    schemaVersion = "math5_rich_raw_extract_v1"
    generatedAt = (Get-Date).ToUniversalTime().ToString("o")
    sourceRoot = $SourceRoot
    assetPublicRoot = "/assets/math5/formulas"
    sources = $Sources
  }
...
try {
  ...
  foreach ($file in $files) {
    ...
    $assetPublicBase = "/assets/math5/formulas/$sourceSlug"
